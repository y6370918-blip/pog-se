...
const NONE = "none";
const RU_GE = "ru_ge";   // Russia MO: must attack German (NE front)
const RU_AH = "ru_ah";   // Russia MO: must attack Austria-Hungary
const ARMY = "army";
const CORPS = "corps";
...
function nation_name(nation) {
  switch (nation) {
    ...
    case NONE:
      return "None";
    case AH_IT:
      return "Austria-Hungary (Italy)";
    case RU_GE:
      return "Russia (Germany/NE)";
    case RU_AH:
      return "Russia (Austria-Hungary)";
    case "allied":
      return "Allied";
    case "sn":
      return "";
...
const all_pieces_by_nation = object_group_by(
  all_pieces,
  (p) => data.pieces[p].nation,
);
const all_pieces_by_faction = object_group_by(
  all_pieces,
  (p) => data.pieces[p].faction,
);

const all_spaces = Array.from(data.spaces, (_, ix) => ix);
const all_spaces_by_nation = object_group_by(
  all_spaces,
  (s) => data.spaces[s].nation,
);
const all_capitals = all_spaces.filter((s) => data.spaces[s].capital);
const all_capitals_by_nation = object_group_by(
  all_capitals,
  (s) => data.spaces[s].nation,
);
const all_supply_and_capitals = all_spaces.filter(
  (s) => data.spaces[s].capital || data.spaces[s].supply,
);
const all_supply_and_capitals_by_nation = object_group_by(
  all_supply_and_capitals,
  (s) => data.spaces[s].nation,
);
...
exports.view = function (state, current) {
  ...
  if (!game.rollback) view.rollback = [];
  else
    view.rollback = game.rollback.map((r) => {
      let name = r.turn_start
        ? `Turn ${r.turn} Start`
        : `Turn ${r.turn} ${r.active} Action ${r.action}`;
      return {
        name: name,
        events: r.events,
      };
  ...
  if (current === "Observer" || game.active !== current) {
    ...
    if (typeof inactive === "function") states[game.state].inactive();
    else if (typeof inactive === "string")
      view.prompt = `Waiting for ${faction_name(active_faction())} to ${inactive}.`;
    else view.prompt = `Waiting for ${faction_name(active_faction())}.`;
  } else {
    view.actions = {};
...
exports.setup = function (seed, scenario, options) {
  ...
  log_h1("paths of glory schlieffen edition schlieffen edition");
  ...
  if (globalThis.RTT_FUZZER) game.options.no_supply_warnings = 1;
...
function create_empty_game_state(seed, scenario) {
  return {
    ...
    forts: {
      // data for spaces tells you strength of forts per space
...
function record_score_event(vp, card, turn = game.turn) {
  if (!game.score_events) game.score_events = [];
  if (card) game.score_events.push([turn, vp, card]);
  else game.score_events.push([turn, vp]);
}
...
function set_up_standard_decks(start_with_guns_of_august) {
  for (let i = 1; i < data.cards.length; i++) {
    if (i === GUNS_OF_AUGUST && start_with_guns_of_august) {
      ...
      data.cards[i].commitment === COMMITMENT_MOBILIZATION &&
      data.utils.is_card_allowed_to_deal(i, game.options)
    ) {
...
function set_up_great_war_scenario_decks() {
  for (let i = 1; i < data.cards.length; i++) {
    let c = data.cards[i];
    if (
      great_war_scenario_played_cp_cards.includes(i) ||
      great_war_scenario_played_ap_cards.includes(i)
    )
  ...
  record_score_event(
    -1,
    RAPE_OF_BELGIUM,
    great_war_scenario_turn_for_event(RAPE_OF_BELGIUM),
  );
  record_score_event(
    1,
    REICHSTAG_TRUCE,
    great_war_scenario_turn_for_event(REICHSTAG_TRUCE),
  );
  record_score_event(
    -1,
    LUSITANIA,
    great_war_scenario_turn_for_event(LUSITANIA),
  );
...
function set_up_played_event(c, turn = 1) {
  ...
  if (card_data.remove) game[card_data.faction].removed.push(c);
  else game[card_data.faction].discard.push(c);
...
function deal_ap_cards() {
  ...
  while (game.ap.hand.length < game.options.hand_size) {
    if (game.ap.deck.length === 0) reshuffle_discard(game.ap.deck);
    if (game.ap.deck.length === 0) break;
...
function deal_cp_cards() {
  if (game.cp.shuffle) {
    // Same as AP shuffle above
    reshuffle_discard(game.cp.deck);
    game.cp.shuffle = false;
  }

  while (game.cp.hand.length < game.options.hand_size) {
    if (game.cp.deck.length === 0) reshuffle_discard(game.cp.deck);
    if (game.cp.deck.length === 0) break;
...
function find_n_unused_pieces(nation, name, n) {
  return data.utils.find_n_pieces(
    nation,
    name,
    n,
    (i) => game.location[i] === 0,
...
const AP_MO_TABLE = {
  ...
  2: FRANCE,
  3: BRITAIN,
  4: RU_GE,   // Russia must attack Germany (NE front)
  5: ITALY,
  6: RU_AH,   // Russia must attack Austria-Hungary
  7: NONE,
};
...
function roll_mandated_offensives() {
  ...
  if (ap_mo === ITALY) {
    if (!nation_at_war(ITALY)) {
      ...
      ap_mo = RU_AH;  // fallback to Russian MO when Italy gone
    }
  }
  const is_russian_mo = ap_mo === RUSSIA || ap_mo === RU_GE || ap_mo === RU_AH;
  if (
    is_russian_mo &&
    (all_capitals_occupied(RUSSIA) || game.events.bolshevik_revolution)
  ) {
    ap_mo = NONE;
  }
  ...
  let cp_drm = 0;
  if (game.events.hoffmann > 0) cp_drm = 1;
  let cp_index = cp_roll + cp_drm > 6 ? 6 : cp_roll + cp_drm;
  let cp_mo = CP_MO_TABLE[cp_index];

  if (
    (cp_mo === AUSTRIA_HUNGARY || cp_mo === AH_IT) &&
    all_capitals_occupied(AUSTRIA_HUNGARY)
  ) {
    cp_mo = TURKEY;
  }
  if (cp_mo === TURKEY) {
  ...
  if (
    cp_mo === AH_IT &&
    (!nation_at_war(ITALY) || all_capitals_occupied(ITALY))
  ) {
...
function satisfies_mo(mo, attackers, defenders, space) {
  ...
  let valid_attacker = attackers.find((a) => {
    ...
    if (piece.nation !== attacker_nation) return false;
    if (attacker_nation === BRITAIN && is_minor_british_nation(a)) {
      return false;
    }
    return true;
  });
  if (valid_attacker === undefined) return false;
  ...
  let valid_defender = defenders.find((d) => {
    ...
    if (attacker_nation === FRANCE || attacker_nation === BRITAIN) {
      ...
      return (
        piece.nation === BELGIUM ||
        piece.nation === FRANCE ||
        piece.nation === BRITAIN ||
        piece.nation === US
      );
    } else if (mo === RU_GE) {
      return piece.nation === GERMANY;
    } else if (mo === RU_AH) {
      return piece.nation === AUSTRIA_HUNGARY;
    } else {
      return true;
    }
  });
  if (valid_defender === undefined) return false;

  let location = data.spaces[space].nation;
  const space_map = data.spaces[space].map;
  if (
    attacker_nation === FRANCE ||
    attacker_nation === BRITAIN ||
    attacker_nation === GERMANY
  ) {
  ...
  if (mo === RU_GE || mo === RU_AH) {
    if (location === "ne" || space_map === "neareast") return true;
  }
...
states.confirm_mo = {
  ...
  prompt() {
    if (game[active_faction()].mo === NONE)
      ...
      game.events.french_mutiny > 0 &&
      game[active_faction()].mo === FRANCE
    )
...
function send_to_eliminated_box(p) {
  if (is_unit_reduced(p)) set_delete(game.reduced, p);
...
function for_each_piece_in_space(s, f) {
  for (let p = 1; p < data.pieces.length; ++p) if (game.location[p] === s) f(p);
}

function get_pieces_in_space(s) {
  let pieces = [];
  for (let p = 1; p < data.pieces.length; ++p)
    if (game.location[p] === s) pieces.push(p);
  return pieces;
}
...
function check_rule_violations() {
  ...
  for (let s = 1; s < map_space_count; ++s) {
    if (
      (has_undestroyed_fort(s, AP) || has_undestroyed_fort(s, CP)) &&
      !is_besieged(s) &&
      game.broken_sieges &&
      !set_has(game.broken_sieges, s)
    ) {
      const enemy_pieces = get_pieces_in_space(s).filter(
        (p) => data.pieces[p].faction !== data.spaces[s].faction,
      );
      if (enemy_pieces.length > 0 && !can_besiege(s, enemy_pieces)) {
        violations.push({
          space: s,
          piece: 0,
          rule: "Insufficient strength to begin a siege",
        });
      }
    }
  }
    ...
    for (let s = 1; s < map_space_count; ++s) {
      ...
      if (has_ap_non_russian && has_russian) {
        violations.push({
          space: s,
          piece: 0,
          rule: "After Brest-Litovsk, RU may not stack with AP units",
        });
      }
    }
  }
...
function can_play_neutral_entry() {
  for (let action of game.ap.actions) {
    if (action.type === ACTION_NEUTRAL_ENTRY) return false;
  }
...
function set_trench_level(s, level, faction) {
  if (faction === undefined) faction = is_controlled_by(s, AP) ? AP : CP;
  if (level) map_set(game[faction].trenches, s, level);
  else map_delete(game[faction].trenches, s);
}
...
function get_trench_level_for_attack(s, faction) {
  ...
  if (!game.attack) return base_lvl;

  if (game.attack.trenches_canceled) return 0;

  if (game.attack.turk_determination) return 1;
...
states.action_phase = {
  ...
  prompt() {
    ...
    if (
      game.turn === 1 &&
      use_historical_scenario_rules() &&
      player_actions.length === 0 &&
      active_faction() === CP
    ) {
      view.prompt += ` You must play "Guns of August"!`;
      gen_card_menu(GUNS_OF_AUGUST, true);
    } else {
      const french_mutiny_active =
        game.events.french_mutiny > 0 && game[active_faction()].mo === FRANCE;
      if (
        player_actions.length === 5 &&
        game[active_faction()].mo !== NONE &&
        !french_mutiny_active
      )
  ...
  play_event(card) {
    push_undo();
    if (!is_ops_event_card(card)) remove_failed_entrench_markers();
...
function gen_card_menu(card, event_only) {
  ...
  if (
    (card === ROMANIA_ENTRY || card === ITALY_ENTRY) &&
    game.cp.commitment === COMMITMENT_TOTAL &&
    game.ap.commitment !== COMMITMENT_TOTAL
  )
    event_only = true;
  ...
  if (event_only) {
    if (can_play_event(card)) gen_action("play_event", card);
  } else {
    if (can_play_event(card)) gen_action("play_event", card);
    gen_action("play_ops", card);
    if (can_play_sr()) gen_action("play_sr", card);
    if (can_play_rps()) gen_action("play_rps", card);
  }
}
...
function can_play_event(card) {
  ...
  if (card_data.reinfnation) {
    ...
    if (
      game.events.uboats_unleashed > 0 &&
      !game.events.convoy &&
      card_data.reinfnation === US
    )
      return false;

    if (card === MEF && !nation_at_war(TURKEY)) return false;
...
function can_play_rps() {
  if (game.turn === game.events.influenza) return false;
...
function goto_play_event(card) {
  ...
  if (data.cards[card].remove) active_player.removed.push(card);
  else active_player.discard.push(card);

  const card_data = data.cards[card];
  if (card_data.ws) {
    active_player.ws += card_data.ws;
    logi(
      `War Status +${card_data.ws} to ${active_player.ws} (${game.ap.ws + game.cp.ws})`,
    );
    update_us_entry();
  }

  let evt = events[card_data.event];
  if (evt.is_neutral_entry) record_action(ACTION_NEUTRAL_ENTRY, card);
  else record_action(ACTION_EVENT, card);
  evt.play();
}
...
states.choose_sr_unit = {
  ...
  prompt() {
    ...
    if (rule_violations.length === 0) {
      if (game.sr.pts > 0) gen_action("confirm_end_sr");
      else gen_action("end_action");
    } else {
...
function can_sr(p) {
  ...
  if (
    game.location[p] === AP_RESERVE_BOX ||
    ...
    if (
      piece_data.nation !== BELGIUM &&
      piece_data.nation !== SERBIA &&
      any_capital_occupied_or_besieged(piece_data.nation)
    ) {
      return false;
    }
  } else {
  ...
  if (is_besieged(game.location[p])) {
    let other_units = get_pieces_in_space(game.location[p]).filter(
      (unit) => unit !== p,
    );
    if (!can_besiege(game.location[p], other_units)) return false;
  }
...
states.choose_sr_destination = {
  ...
  prompt() {
    ...
    if (destinations.length === 0) {
      ...
      if (globalThis.RTT_FUZZER) gen_action_skip();
    }
  },
  space(s) {
    set_ne_restriction_flags_for_sr(
      game.sr.unit,
      game.location[game.sr.unit],
      s,
    );
    ...
    if (from === AP_RESERVE_BOX || from === CP_RESERVE_BOX)
      log(
        `${piece_name(game.sr.unit)}${log_corps(game.sr.unit)} SR\n${space_name(from)} -> ${space_name(s)}`,
      );
    else
      log(
        `${piece_name(game.sr.unit)} SR\n${space_name(from)} -> ${space_name(s)}`,
      );
    game.sr.unit = 0;
    game.state = "choose_sr_unit";
    update_siege(from);
...
function get_sr_destinations(unit) {
  ...
  if (unit === MONTENEGRIN_CORPS) {
    if (start === AP_RESERVE_BOX && is_controlled_by(CETINJE, AP))
      return [CETINJE];
    if (start === CETINJE) return [AP_RESERVE_BOX];
    return [];
  }
  ...
  if (start === AP_RESERVE_BOX || start === CP_RESERVE_BOX) {
    ...
    for (let p of all_pieces_by_nation[nation]) {
      ...
      if (
        ...
        if (is_bef_unit(unit) && !can_bef_enter(s)) continue;
    ...
    if (
      nation === SERBIA &&
      is_unit_supplied_in(unit, SALONIKA) &&
      is_controlled_by(SALONIKA, AP)
    ) {
    ...
    destinations = destinations.filter((s) =>
      is_space_supplied_for_reserve_box_sr(s, unit),
    );
  } else if (
    data.pieces[unit].type === CORPS &&
    is_space_supplied_for_reserve_box_sr(game.location[unit], unit)
  ) {
  ...
  let frontier = [start];
  const block_neareast =
    !is_neareast_space(start) && !can_enter_neareast([unit]);
  while (frontier.length > 0) {
    let current = frontier.pop();
    get_connected_spaces(current, nation).forEach((n) => {
      if (
        !set_has(destinations, n) &&
        is_unit_supplied_in(unit, n) &&
        (is_controlled_by(n, faction) || is_besieged(n))
      ) {
        if (nation === RUSSIA && data.spaces[n].nation !== RUSSIA) return;

        if (
          is_blocked_italian_space(n, [unit]) ||
          is_blocked_italian_border_space(n, [unit])
        )
          return;
        ...
        if (block_neareast && is_neareast_space(n)) return;

        if (is_bef_unit(unit) && !can_bef_enter(n)) return;
  ...
  if (
    data.pieces[unit].nation !== BELGIUM &&
    data.pieces[unit].nation !== SERBIA &&
    any_capital_occupied_or_besieged(data.pieces[unit].nation)
  ) {
  ...
  for (let d of all_destinations) {
    ...
    if (
      game.ne_restrictions.cp_sr &&
      data.pieces[unit].faction === CP &&
      nation !== TURKEY
    ) {
    ...
    if (
      data.pieces[unit].nation === RUSSIA &&
      data.pieces[unit].type === ARMY
    ) {
      set_delete(destinations, d);
      continue;
    }
    ...
    if (!set_has(overland_destinations, d)) {
      ...
      if (
        [ITALY, FRANCE, GREECE, ROMANIA, SERBIA, US, BELGIUM].includes(
          data.pieces[unit].nation,
        )
      )
        set_delete(destinations, d);
    }
  ...
  if (game.events.treaty_of_brest_litovsk > 0) {
    destinations = destinations.filter(
      (d) => !is_brest_litovsk_restricted_one(unit, d),
    );
  }
...
function update_russian_ne_restriction_flag(units, source, destination) {
  ...
  if (ru_corps_count > 0) {
    if (
      (source === CAUCASUS && is_neareast_space(destination)) ||
...
function check_russian_ne_restriction(units, destination) {
  ...
  for (let u of units) {
    if (
      (is_neareast_space(game.location[u]) && destination === CAUCASUS) ||
  ...
  if (ru_corps_crossing_count === 0) return true;

  // Once Fall of the Tsar has been played, no Russian corps can cross this boundary by move, advance, or retreat
  if (game.events.fall_of_the_tsar > 0) return false;

  if (ru_corps_crossing_count > 1) return false;

  if (ru_corps_crossing_count === 1) return !game.ne_restrictions.ru_non_sr;
}
...
function is_brest_litovsk_restricted_one(p, destination) {
  ...
  if (
    game.events.treaty_of_brest_litovsk > 0 &&
    data.pieces[p].faction === AP
  ) {
    return (
      (data.pieces[p].nation === RUSSIA &&
        has_non_russian_ap_piece(destination)) ||
...
function is_brest_litovsk_restricted_group(pieces, destination) {
  if (
    game.events.treaty_of_brest_litovsk > 0 &&
    data.pieces[pieces[0]].faction === AP
  ) {
    return (
      (pieces.some((p) => data.pieces[p].nation === RUSSIA) &&
        has_non_russian_ap_piece(destination)) ||
      (pieces.some((p) => data.pieces[p].nation !== RUSSIA) &&
        has_russian_piece(destination))
    );
  }
  return false;
}
...
function goto_play_rps(card) {
  ...
  game.rp.bu +=
    card_data.rpbu !== undefined && nation_at_war(BULGARIA)
      ? card_data.rpbu
      : 0;
  game.rp.it +=
    card_data.rpit !== undefined && nation_at_war(ITALY) ? card_data.rpit : 0;
  game.rp.tu +=
    card_data.rptu !== undefined && nation_at_war(TURKEY) ? card_data.rptu : 0;
  ...
  if (active_faction() === AP) {
    ["fr", "br", "ru", "it", "us", "allied"].forEach((type) => {
      if (game.rp[type] > 0) logi(`${nation_name(type)}: ${game.rp[type]}`);
    });
  } else {
...
function goto_play_reinf(card) {
  ...
  array_remove_item(active_player.hand, card);
  game.last_card = card;
  if (card_data.remove) active_player.removed.push(card);
  else active_player.discard.push(card);
  ...
  if (!game.events.reinforcements) game.events.reinforcements = [];
  game.events.reinforcements.push(card);

  let piece_nation = card_data.reinfnation;
  if (card === LIBYAN_REVOLT) piece_nation = "sn"; // This card counts as Turkish reinforcements but places the 'sn' piece
  if (card === ARAB_NORTHERN_ARMY) piece_nation = "ana"; // This card counts as British reinforcements but places the 'ana' piece
...
states.place_reinforcements = {
  ...
  space(s) {
    ...
    if (
      neareast_armies.includes(p) &&
      data.spaces[s].map !== "neareast" &&
      !is_mef_space(s) &&
      s !== SALONIKA
    ) {
      log(
        `${piece_name(p)} is a NE army placed outside the Near East, it will not be able to operate on the Near East map`,
      );
...
function get_available_reinforcement_spaces(p) {
  ...
  if (p === TURKISH_SN_CORPS) {
    ...
    for_each_piece_in_space(LIBYA, (p) => {
      if (data.pieces[p].faction === AP) libya_available = false;
    });
  ...
  if (piece_data.type === CORPS) {
    if (piece_data.faction === AP) spaces.push(AP_RESERVE_BOX);
    else spaces.push(CP_RESERVE_BOX);
    return spaces;
  }

  // Special placement options for French Orient Army, British NE Army, Russian CAU Army, and British MEF Army
  if (
    piece_data.name === "FR Orient" &&
    is_space_at_war(SALONIKA) &&
    !is_fully_stacked(SALONIKA, active_faction()) &&
    is_controlled_by(SALONIKA, AP)
  ) {
    spaces.push(SALONIKA);
  } else if (
    piece_data.name === "BR NE" &&
    game.events.sinai_pipeline > 0 &&
    !is_fully_stacked(ALEXANDRIA, active_faction()) &&
    is_controlled_by(ALEXANDRIA, AP)
  ) {
    ...
    for (let s of all_spaces_by_nation[RUSSIA]) {
      if (
        data.spaces[s].map === "neareast" &&
        is_unit_supplied_in(p, s) &&
        !is_fully_stacked(s, AP) &&
  ...
  for (let c of capitals) {
    if (
      is_controlled_by(c, active_faction()) &&
      is_unit_supplied_in(p, c) &&
      !is_fully_stacked(c, active_faction())
    ) {
      spaces.push(c);
    }
  }
  ...
  for (let s of all_spaces_by_nation[nation]) {
    if (
      data.spaces[s].supply &&
      is_controlled_by(s, active_faction()) &&
      is_unit_supplied_in(p, s) &&
      !is_fully_stacked(s, active_faction())
    ) {
      spaces.push(s);
    }
  }
...
function can_activate_piece_in_space_to_attack(p, s, used_ne_activation) {
  ...
  if (active_faction() === AP && used_ne_activation && is_neareast_space(s)) {
    ...
    if (is_mef_space(s) || game.location[BRITISH_NE_ARMY] === s) {
      if (get_attackable_spaces([p]).length > 0) return true;
    }
    return false;
  }
  ...
  if (s === LONDON || get_attackable_spaces([p]).length > 0) return true;
...
states.activate_spaces = {
  ...
  prompt() {
    ...
    const used_ne_activation = game.activated.attack.some(
      (s) =>
        is_neareast_space(s) &&
        !is_mef_space(s) &&
        s !== game.location[BRITISH_NE_ARMY],
    );
    ...
    for (let p of all_pieces_by_faction[active_faction()]) {
      if (set_has(game.oos_pieces, p)) continue;

      let s = game.location[p];
      if (s === 0 || s >= AP_RESERVE_BOX || !is_space_at_war(s)) continue;
      ...
      if (
        !set_has(attack_spaces, s) &&
        game.ops >= cost_to_activate(s, ATTACK) &&
        can_activate_piece_in_space_to_attack(p, s, used_ne_activation)
      )
        set_add(attack_spaces, s);
    }

    for (let s of move_spaces) gen_action("activate_move", s);
    ...
    for (let s of attack_spaces) {
      ...
      if (active_faction() === AP && is_french_mutiny_mo()) {
        ...
        if (nation === FRANCE || nation === BELGIUM || nation === GERMANY) {
          if (
            all_pieces_by_nation[FRANCE].some((p) => game.location[p] === s) &&
  ...
  activate_move(s) {
    ...
    if (
      !game.sud_army_space &&
      is_possible_sud_army_stack(get_pieces_in_space(s))
    ) {
      game.sud_army_space = s;
    }
    if (game.ops === 0) start_action_round();
  },
...
function next_attack_activation() {
  ...
  game.eligible_attackers = game.eligible_attackers.filter(
    (p) => game.location[p] === LONDON || get_attackable_spaces([p]).length > 0,
  );

  // remove attack markers with no eligible attackers remaining
  game.activated.attack = game.activated.attack.filter((s) =>
    game.eligible_attackers.some((p) => game.location[p] === s),
  );
...
function end_move_activation() {
  ...
  if (game.activated.move.length + game.entrenching.length > 0) update_supply(); // TODO -- move to after each stack moves instead for more granularity?
  if (game.move) {
...
function end_attack_activation() {
  if (game.attack.used_peace_offensive && !game.attack.did_advance) {
    log(
      `${faction_name(active_faction())} failed to advance after using ${card_name(PEACE_OFFENSIVE)}, -1 VP`,
    );
    game.vp--;
  ...
  if (game.eligible_attackers.length === 0) game.activated.attack = [];
...
function remove_failed_entrench_markers() {
  game.failed_entrench = game.failed_entrench.filter(
    (p) => data.pieces[p].faction !== active_faction(),
  );
}
...
function goto_trench_rolls() {
  ...
  if (game.entrenching.length > 0) game.state = "trench_rolls";
  else next_attack_activation();
}
...
states.trench_rolls = {
  ...
  prompt() {
    ...
    for (let p of game.entrenching) gen_action_space(game.location[p]);
  },
  space(s) {
  ...
  piece(p) {
    ...
    let drm = 0;
    if (game.failed_entrench) {
      if (set_has(game.failed_entrench, p)) drm = -1;
    }
    ...
    if (success) {
      ...
      if (game.failed_entrench) set_delete(game.failed_entrench, p);
    } else {
    ...
    if (game.entrenching.length === 0) next_attack_activation();
  },
};
...
function goto_end_action() {
  ...
  if (active_faction() === AP && game.events.high_seas_fleet > 0) {
    log(
      `${faction_name(AP)} did not play ${card_name(GRAND_FLEET)} this action round, ${card_name(HIGH_SEAS_FLEET)} +1 VP`,
    );
    game.vp++;
...
function update_us_entry() {
  ...
  if (game.events.over_there > 0) {
    game.us_entry = 3;
    if (previous_level < 2)
      log_h3(`US Commitment Track has reached ${card_name(OVER_THERE)}!`); // (The ! is verbatim from the playing board)
  } else if (game.events.zimmermann_telegram > 0) {
    game.us_entry = 2;
    if (previous_level < 2)
      log_h3(
        `US Commitment Track has reached ${card_name(ZIMMERMANN_TELEGRAM)}`,
      );
  } else if (events.zimmermann_telegram.can_play()) {
    game.us_entry = 1;
    if (previous_level < 1)
      log_h3(
        `US Commitment: ${card_name(ZIMMERMANN_TELEGRAM)} can now be played`,
      );
  } else {
    game.us_entry = 0;
  }
...
function update_russian_capitulation() {
  ...
  if (game.events.treaty_of_brest_litovsk > 0) {
    ...
    if (previous_level < 7) {
      log_h3(
        `Russian Capitulation Track reaches ${card_name(TREATY_OF_BREST_LITOVSK)}`,
      );
    }
    ...
    if (previous_level < 6) {
      log_h3(
        `Russian Capitulation Track reaches ${card_name(BOLSHEVIK_REVOLUTION)}`,
      );
    }
    ...
    if (previous_level < 5)
      log_h3(
        `Russian Capitulation: ${card_name(BOLSHEVIK_REVOLUTION)} can now be played`,
      );
  } else if (game.events.fall_of_the_tsar > 0) {
    game.russian_capitulation = 4;
    if (previous_level > 4) {
      log_h3(
        `Russian Capitulation: ${card_name(BOLSHEVIK_REVOLUTION)} can no longer be played`,
      );
    } else if (previous_level < 4) {
      log_h3(
        `Russian Capitulation Track reaches ${card_name(FALL_OF_THE_TSAR)}`,
      );
    }
    ...
    if (previous_level < 3)
      log_h3(
        `Russian Capitulation: ${card_name(FALL_OF_THE_TSAR)} can now be played`,
      );
  } else if (game.events.tsar_takes_command > 0) {
    game.russian_capitulation = 2;
    if (previous_level > 2) {
      log_h3(
        `Russian Capitulation: ${card_name(FALL_OF_THE_TSAR)} can no longer be played`,
      );
    } else if (previous_level < 2) {
      log_h3(
        `Russian Capitulation Track reaches ${card_name(TSAR_TAKES_COMMAND)}`,
      );
    }
    ...
    if (previous_level < 1)
      log_h3(
        `Russian Capitulation: ${card_name(TSAR_TAKES_COMMAND)} can now be played`,
      );
  } else {
    game.russian_capitulation = 0;
    if (previous_level > 0)
      log_h3(
        `Russian Capitulation: ${card_name(TSAR_TAKES_COMMAND)} can no longer be played`,
      );
  }
}
...
states.choose_move_space = {
  ...
  prompt() {
    ...
    else view.prompt = `Move units in spaces activated for movement.`;
...
function can_entrench_with_selected() {
  if (!can_entrench()) return false;
  if (game.move.spaces_moved > 0 || game.move.pieces.length !== 1) return false;
...
states.choose_pieces_to_move = {
  ...
  prompt() {
    ...
    else view.prompt = `Move units from ${space_name(game.move.initial)}.`;
    for_each_piece_in_space(game.move.initial, (p) => {
      if (
        get_piece_mf(p) > 0 &&
        !set_has(game.entrenching, p) &&
        !set_has(game.moved, p) &&
        !set_has(game.move.pieces, p) &&
        !set_has(game.oos_pieces, p)
      ) {
        gen_action_piece(p);
      }
    ...
    if (game.move.pieces.length > 0) {
      ...
      if (spaces.length === 0) gen_action("stop");
      spaces.forEach(gen_action_space);
    } else {
      view.actions.done = 1;
    }
  ...
  entrench() {
    ...
    if (
      get_pieces_in_space(game.location[p]).every(
        (piece) =>
          set_has(game.entrenching, piece) || set_has(game.moved, piece),
      )
    )
      end_move_activation();
  },
  ...
  space(s) {
    ...
    if (must_stop_moving(s)) end_move_stack();
    else game.state = "move_stack";
  },
  done() {
    push_undo();
    end_move_activation();
...
function get_eligible_spaces_to_move() {
  if (is_besieged(game.move.current)) {
    ...
    for_each_piece_in_space(game.move.current, (p) => {
      if (!set_has(game.move.pieces, p)) units_in_space_after_move.push(p);
    });
    if (
      units_in_space_after_move.length > 0 &&
      !can_besiege(game.move.current, units_in_space_after_move)
    )
  ...
  if (game.move.spaces_moved < lowest_mf) {
    let moving_nations = [];
    game.move.pieces.forEach((p) => {
      set_add(moving_nations, data.pieces[p].nation);
    });
    ...
    connections.forEach((conn) => {
      const blocked_end_space =
        is_last_space && !can_end_move(conn, game.move.pieces);
...
function can_take_control(pieces) {
  ...
  if (pieces.length === 1 && pieces[0] === BRITISH_ANA_CORPS) return false;
  return true;
}
...
function move_stack_to_space(s) {
  if (is_besieged(game.move.current)) {
    ...
    for_each_piece_in_space(game.move.current, (p) => {
      if (!set_has(game.move.pieces, p)) pieces_remaining.push(p);
    });
  ...
  if (!has_undestroyed_fort(s, other_faction(active_faction()))) {
    if (can_take_control(game.move.pieces)) set_control(s, active_faction());
  }
}
...
states.move_stack = {
  ...
  space(s) {
    ...
    let to_stop = game.move.pieces.filter(
      (p) => get_piece_mf(p) === game.move.spaces_moved,
    );
    for (let p of to_stop) drop_off_moving_piece(p);

    if (game.move.pieces.length === 0) end_move_stack();
  },
  piece(p) {
    push_undo();
...
function set_control_bit(i, x) {
  var word = i >> 5;
  var bit = i & 31;
  if (x) game.control[word] |= 1 << bit;
  else game.control[word] &= ~(1 << bit);
}
...
function set_control(s, faction) {
  ...
  if (get_control_bit(s) === new_control) return;
  ...
  if (
    s === game.mef_beachhead &&
    !game.mef_beachhead_captured &&
    faction === CP
  ) {
...
function update_ana_vp() {
  if (game.ana_prev === game.location[BRITISH_ANA_CORPS]) return;

  // ANA was in this space, but it's now CP controlled. Update VP.
  if (
    game.ana_prev &&
    is_controlled_by(game.ana_prev, CP) &&
    data.spaces[game.ana_prev].vp
  ) {
    game.vp++;
  ...
  if (
    get_control_bit(game.location[BRITISH_ANA_CORPS]) === 1 &&
    is_controlled_by(game.location[BRITISH_ANA_CORPS], AP) &&
    data.spaces[game.location[BRITISH_ANA_CORPS]].vp
  ) {
    game.vp--;
...
function contains_piece_of_nation(s, nation) {
  for (let p of all_pieces_by_nation[nation])
    if (game.location[p] === s) return true;
...
function contains_only_pieces_of_nation(s, nation) {
  ...
  for (let p = 1; p < data.pieces.length; ++p)
    if (game.location[p] === s) {
      if (data.pieces[p].nation === nation) found_piece = true;
      else return false;
    }
  return found_piece;
}
...
function can_move_to(s, moving_pieces) {
  let contains_enemy = contains_piece_of_faction(
    s,
    other_faction(active_faction()),
  );
  if (contains_enemy) return false;

  if (is_blocked_italian_space(s, moving_pieces)) return false;
...
function can_enter_neareast(pieces) {
  for (let p of pieces) {
    ...
    if (
      data.pieces[p].type === ARMY &&
      (!neareast_armies.includes(p) ||
        set_has(game.ne_armies_placed_outside_neareast, p))
    ) {
      return false;
    }
...
function is_fully_stacked(s, faction) {
  if (s === AP_RESERVE_BOX || s === CP_RESERVE_BOX) return false;
  ...
  for (let p = 1; p < game.location.length; ++p) {
    ...
    if (matches === STACKING_LIMIT) return true;
  }
  return false;
}
...
function can_end_move(s, pieces) {
  if (set_has(game.activated.attack, s)) return false;

  if (
    active_faction() === CP &&
    !game.events.race_to_the_sea &&
    (s === AMIENS || s === CALAIS || s === OSTEND) &&
    game.cp.ws < 4
  )
    return false;

  if (is_blocked_italian_border_space(s, pieces)) return false;

  return true;
}

function is_blocked_italian_space(s, pieces) {
  if (game.ap.commitment === COMMITMENT_TOTAL) return false; // Once AP is at Total War, there are no restrictions
...
function is_blocked_italian_border_space(s, pieces) {
  ...
  const has_german_army = pieces.some((p) => {
    return data.pieces[p].nation === GERMANY && data.pieces[p].type === ARMY;
  });
  return has_german_army && italian_border_spaces.includes(s);
}

function end_move_stack() {
  if (game.move.pieces.length > 0) log_piece_move(game.move.pieces);
  ...
  for (let p of game.move.pieces) set_add(game.moved, p);
...
function get_defenders_pieces() {
  ...
  for (let p = 1; p < data.pieces.length; ++p)
    if (
      ...
      data.pieces[p].faction !== game.attack.attacker
    )
      pieces.push(p);
  return pieces;
}
...
states.choose_attackers = {
  ...
  prompt() {
    ...
    const attacking_from_siege = game.eligible_attackers.some((p) =>
      is_besieged(game.location[p]),
    );
    if (
      !attacking_from_siege &&
      game.attack.pieces.length === 0 &&
      get_attackable_spaces(game.eligible_attackers).length > 0
    ) {
      gen_action("select_all");
    }

    if (game.attack.pieces.length === 0) {
      if (game.eligible_attackers.length > 0) gen_action("confirm_pass_attack");
      else gen_action_pass();
    }
  },
  no_attack() {
    push_undo();
    for (let p of game.attack.pieces) set_delete(game.eligible_attackers, p);
    game.attack.pieces = [];
...
function fmt_attack_odds() {
  ...
  const attack_factors = game.attack.pieces.reduce(
    (sum, p) => sum + get_piece_cf(p),
    0,
  );

  const defender_pieces = get_defenders_pieces();
  let defense_factors = defender_pieces.reduce((sum, p) => {
    if (set_has(game.retreated, p)) return sum;
    return sum + get_piece_cf(p);
  }, 0);

  const attacker_table = game.attack.pieces.some(
    (p) => data.pieces[p].type === ARMY,
  )
    ? fire_table.army
    : fire_table.corps;
  const defender_table = defender_pieces.some(
    (p) => data.pieces[p].type === ARMY && !set_has(game.retreated, p),
  )
    ? fire_table.army
    : fire_table.corps;
  ...
  const trench_shift_canceled =
    attacking_unoccupied_fort() ||
    game.attack.trenches_canceled ||
    game.attack.trench_shift_canceled;
  ...
  const attacker_col = get_fire_col(
    attacker_table,
    attack_factors,
    attacker_shifts,
  );
  const defender_col = get_fire_col(
    defender_table,
    defense_factors,
    defender_shifts,
  );
...
function french_mutiny_penalty_should_be_awarded() {
  ...
  if (
    !is_french_mutiny_mo() ||
    (nation !== FRANCE && nation !== BELGIUM && nation !== GERMANY)
  ) {
    return false;
  }
  ...
  return game.attack.pieces
    .filter((p) => data.pieces[p].nation === FRANCE)
...
function goto_attack() {
  ...
  let attack_sources = [];
  game.attack.pieces.forEach((p) => {
    set_add(attack_sources, game.location[p]);
  });
  attack_sources.forEach((source) => {
    update_russian_ne_restriction_flag(
      game.attack.pieces,
      source,
      game.attack.space,
    );
  ...
  if (game.attack.attacker === AP && is_french_mutiny_mo()) {
    if (french_mutiny_penalty_should_be_awarded()) {
      game.vp += 1;
      log_h3(
        `${card_name(FRENCH_MUTINY)} -- +1 VP -- French unit not stacked with US unit attacked a space in France/Belgium/Germany during French Mutiny`,
      );
      game[active_faction()].mo = NONE;
  ...
  if (
    has_undestroyed_fort(game.attack.space, other_faction(game.attack.attacker))
  ) {
    if (defenders.length > 0) logi(`${piece_list(defenders)}, Fort`);
    else logi(`Fort`);
  } else {
  ...
  if (
    mo !== NONE &&
    (game.attack.attacker !== AP || !is_french_mutiny_mo()) &&
    satisfies_mo(mo, game.attack.pieces, defenders, game.attack.space)
  ) {
    game[active_faction()].mo = NONE;
...
function goto_attack_step_great_retreat() {
  if (
    game.turn === game.events.great_retreat &&
    get_defenders_pieces().some((p) => data.pieces[p].nation === RUSSIA)
  ) {
    clear_undo();
    switch_active_faction();
...
function goto_attack_step_brusilov_offensive() {
  if (
    game.action_state.brusilov_available &&
    events.brusilov_offensive.can_apply()
  ) {
...
function goto_attack_step_trench_negation() {
  if (
    get_trench_level_for_attack(
      game.attack.space,
      other_faction(game.attack.attacker),
    ) > 0 &&
...
function has_trench_negating_card_in_hand(faction) {
  ...
  for (let c of TRENCH_NEGATING_CARDS) {
    if (hand.includes(c) && can_play_event(c)) return true;
  }
  return false;
}
...
function goto_attack_step_kerensky_offensive() {
  if (
    game.action_state.kerensky_available &&
    events.kerensky_offensive.can_apply()
  ) {
...
function goto_attack_step_combat_cards() {
  if (could_have_usable_combat_card(game.attack.attacker, true)) {
    ...
    get_defenders_pieces().some((p) => !set_has(game.oos_pieces, p)) &&
    could_have_usable_combat_card(other_faction(game.attack.attacker))
  ) {
    clear_undo();
...
function could_have_usable_combat_card(faction, skip_deck) {
  ...
  for (let c of game[faction].hand) {
    if (could_apply_combat_card(c)) return true;
  }
...
function attacking_unoccupied_fort() {
  let unretreated_defenders = get_defenders_pieces().filter(
    (p) => !set_has(game.retreated, p),
  );
  return (
    data.spaces[game.attack.space].fort > 0 &&
...
function attacker_can_flank() {
  ...
  let attack_spaces = [];
  game.attack.pieces.forEach((p) => {
    if (data.pieces[p].type === ARMY) has_army = true;
    set_add(attack_spaces, game.location[p]);
  });
  if (!has_army || attack_spaces.length < 2) return false;
...
function can_withdraw() {
  return (
    get_retreat_options(get_defenders_pieces(), game.attack.space, 1).length > 0
  );
}
...
function get_attackable_spaces(attackers) {
  ...
  for (let i = 0; i < attackers.length; ++i) {
    ...
    if (i === 0)
      // First attacker's spaces are all eligible
      eligible_spaces.push(...attackable_spaces); // Subsequent attackers subtract ineligible spaces
    else
      eligible_spaces = eligible_spaces.filter((s) =>
        attackable_spaces.includes(s),
      );
  }

  // 15.1.3 If all the attackers are besieging a space, then they can attack the space they are besieging
  if (
    is_besieged(game.location[attackers[0]]) &&
    attackers.every((p) => game.location[p] === game.location[attackers[0]])
  ) {
    const siege_space = game.location[attackers[0]];
    const remaining_besieging_pieces = get_pieces_in_space(siege_space).filter(
      (p) => !attackers.includes(p),
    );
    ...
    else eligible_spaces = [siege_space]; // Insufficient force remains to maintain the siege, so the only eligible target is the besieged space
  }
  ...
  let besieged_spaces = attackers
    .map((p) => game.location[p])
    .filter((s) => is_besieged(s));
  for (let besieged_space of besieged_spaces) {
    ...
    const remaining_besieging_pieces = get_pieces_in_space(
      besieged_space,
    ).filter((p) => !attackers.includes(p) && is_eligible_for_siege(p));
  ...
  const attacking_from_london = attackers.some(
    (p) => game.location[p] === LONDON,
  );
  if (attacking_from_london) {
    const attacking_from_france_belgium = attackers.some((p) => {
      const loc = game.location[p];
      return (
        data.spaces[loc].nation === FRANCE ||
        data.spaces[loc].nation === BELGIUM
      );
    });
  ...
  const russian_attacker = attackers.some(
    (p) => data.pieces[p].nation === RUSSIA,
  );
  const german_attacker = attackers.some(
    (p) => data.pieces[p].nation === GERMANY,
  );
  const lloyd_george =
    is_lloyd_george_active() &&
    attackers.some((p) => data.pieces[p].nation === BRITAIN);
  const stavka_timidity =
    game.turn === game.events.stavka_timidity && russian_attacker;
  const is_summer = get_season() === SEASON_SUMMER;
  eligible_spaces = eligible_spaces.filter((s) => {
    // Neutral nations
    if (!is_space_at_war(s)) return false;

    if (is_summer && data.spaces[s].terrain === DESERT) return false;

    // Already attacked this turn
    if (game.attacked && set_has(game.attacked, s)) return false;

    // Until AP is at Total War, only Italian and Austrian units may attack into Italian spaces
    if (is_blocked_italian_space(s, attackers)) return false;
    ...
    if (
      german_attacker &&
      data.spaces[s].nation === RUSSIA &&
      has_undestroyed_fort(s, AP)
    ) {
      if (game.cp.ws < 4 && game.events.oberost === undefined) return false;
    }

    if (!check_russian_ne_restriction(attackers, s)) return false;

    // Lloyd George prevents attacks against German defenders with level 2 trenches while active
    if (
      lloyd_george &&
      get_trench_level(s, CP) === 2 &&
      contains_piece_of_nation(s, GERMANY)
    )
      return false;
    ...
    if (
      stavka_timidity &&
      get_trench_level(s, CP) > 0 &&
      contains_only_pieces_of_nation(s, GERMANY)
    )
      return false;
...
function is_eligible_for_siege(p) {
  if (!game.ineligible_for_siege) return true;
...
function is_invalid_multinational_attack(attackers) {
  ...
  for (let p of attackers) {
    ...
    if (nations_in_space[space] === undefined) nations_in_space[space] = [];
    set_add(nations_in_space[space], nation); // Collect unique nations per space
  }

  if (all_nations.length <= 1) return false; // Not a multinational attack
  ...
  for (const s in nations_in_space) {
    ...
    if (nations_in_space[s].length >= all_nations.length) return false;
  }
  return true;
}
...
function has_non_russian_ap_piece(s) {
  for (let p of all_pieces_by_faction[AP])
    if (data.pieces[p].nation !== RUSSIA && game.location[p] === s) return true;
  return false;
}
...
function get_attackable_spaces_for_piece(p) {
  ...
  for (let conn of get_connected_spaces(
    game.location[p],
    data.pieces[p].nation,
  )) {
...
function is_attackable_space_for_piece(s, p) {
  ...
  if (game.events.treaty_of_brest_litovsk > 0) {
    ...
    if (data.pieces[p].faction === CP) {
      if (
        (data.pieces[p].nation !== TURKEY || !is_neareast_space(s)) &&
        has_russian_piece(s)
      )
        return false;
    }
  ...
  if (is_bef_unit(p) && !can_bef_enter(s)) return false;

  // Check if space has an attackable fort
  if (
    has_undestroyed_fort(s, other_faction(active_faction())) &&
    is_controlled_by(s, other_faction(active_faction())) &&
    !is_besieged(s)
  ) {
    return true;
  }
  ...
  if (game.retreated) {
    const defending_pieces = get_pieces_in_space(s).filter(
      (p) => data.pieces[p].faction !== active_faction(),
    );
    if (
      defending_pieces.length > 0 &&
      defending_pieces.every((p) => set_has(game.retreated, p))
    ) {
      return false;
    }
  }
...
states.negate_trench = {
  ...
  prompt() {
    game[active_faction()].hand.forEach((c) => {
      if (
        TRENCH_NEGATING_CARDS.includes(c) &&
        events[data.cards[c].event].can_play()
      ) {
        gen_action_card(c);
      }
    });
...
states.choose_flank_attack = {
  ...
  prompt() {
    let flanking_spaces = get_flanking_spaces(
      get_attack_spaces(game.attack.pieces),
      game.attack.space,
...
function can_play_wireless_intercepts() {
  return (
    game[active_faction()].hand.includes(WIRELESS_INTERCEPTS) &&
...
function get_attack_spaces(pieces) {
  let attack_spaces = [];
  for (let p of pieces) set_add(attack_spaces, game.location[p]);
...
function roll_flank_attack() {
  ...
  for (var s of flanking_spaces) logi("+1 " + space_name(s));
...
function is_usable_combat_card(c) {
  if (!data.cards[c].cc) return false;
...
function prompt_combat_cards() {
  for (let c of game[active_faction()].hand) {
    if (is_usable_combat_card(c)) gen_action_card(c);
  }

  for (let c of game.combat_cards) {
    if (
      data.cards[c].faction === active_faction() &&
      !game.attack.combat_cards.includes(c)
    )
      if (is_usable_combat_card(c)) gen_action_card(c);
  }
  ...
  else view.prompt = `You may play combat cards (none possible).`;
  ...
  card(c) {
    ...
    if (game.attack.combat_cards.length > 0) {
      ...
      for (let c of game.attack.combat_cards) {
        if (data.cards[c].faction === active_faction()) logi(`${card_name(c)}`);
      }
    }
    set_active_faction(other_faction(game.attack.attacker));
    if (
      get_defenders_pieces().some((p) => !set_has(game.oos_pieces, p)) &&
      could_have_usable_combat_card(active_faction())
    )
      game.state = "defender_combat_cards";
    else begin_combat();
  },
...
function used_cc_this_round(c) {
  return (
    game.action_state.used_ccs !== undefined &&
    set_has(game.action_state.used_ccs, c)
  );
}
...
function begin_combat() {
  ...
  if (game.attack.combat_cards.length > 0) {
    ...
    game.attack.combat_cards.forEach((c) =>
      set_add(game.action_state.used_ccs, c),
    );
  }
  ...
  if (
    ...
    [FRANCE, BELGIUM, GERMANY].includes(
      data.spaces[game.attack.space].nation,
    ) &&
...
function adds_flanking_drm(space, attacking_faction, attack_space) {
  ...
  for (let i = 0; i < spaces.length; ++i) {
    if (
      spaces[i] !== attack_space &&
      contains_piece_of_faction(spaces[i], other_faction(attacking_faction))
    )
      return false;
  }
  return true;
...
function log_combat_winner() {
  ...
  if (a_hits > d_hits) log(`*${a_hits}:${d_hits} attacker victory`);
  else if (a_hits < d_hits) log(`*${a_hits}:${d_hits} defender victory`);
  else log(`*${a_hits}:${d_hits} no combat winner`);
}
...
function get_fire_col(table, cf, shifts) {
  ...
  while (col < table.length && table[col].factors <= cf) {
    col++;
  } // Find first column where column factors > combat factors
...
function resolve_attackers_fire() {
  ...
  for (let p of game.attack.pieces) {
    ...
    if (data.pieces[p].type === ARMY) game.attack.attacker_table = ARMY;
  }
  ...
  if (
    !attacking_unoccupied_fort() &&
    !(game.attack.trenches_canceled || game.attack.trench_shift_canceled)
  ) {
    if (
      get_trench_level_for_attack(
        game.attack.space,
  ...
  game.attack.combat_cards.forEach((c) => {
    if (data.cards[c].faction === game.attack.attacker) {
      let evt = events[data.cards[c].event];
      if (evt && evt.can_apply !== undefined && evt.can_apply()) evt.apply();
    }
  });

  if (game.action_state.yanks_and_tanks) {
    if (events.yanks_and_tanks.can_apply()) events.yanks_and_tanks.apply();
  }
  ...
  if (
    game.attack.pieces.every((p) => game.location[p] === SINAI) &&
    !(game.attack.attacker === AP && game.events.sinai_pipeline > 0)
  ) {
  ...
  let clamped_roll =
    modified_roll > 6 ? 6 : modified_roll < 1 ? 1 : modified_roll;
  game.attack.defender_losses = get_fire_result(
    game.attack.attacker,
    base_roll,
    game.attack.attacker_table,
    attacker_cf,
    attacker_shifts,
    clamped_roll,
  );
...
function resolve_defenders_fire() {
  ...
  const defender_pieces = get_pieces_in_space(game.attack.space).filter(
    (p) => data.pieces[p].faction === defender,
  );
  defender_pieces.forEach((p) => {
    if (!set_has(game.retreated, p)) {
      defender_cf += get_piece_cf(p);
      if (data.pieces[p].type === ARMY) game.attack.defender_table = ARMY;
    }
  });
  ...
  let defender_shifts = 0;
  let trench_shift_canceled =
    game.attack.trenches_canceled || game.attack.trench_shift_canceled;
  if (
    get_trench_level_for_attack(game.attack.space, defender) > 0 &&
    !trench_shift_canceled &&
    !attacking_unoccupied_fort()
  ) {
    defender_shifts += 1;
  ...
  let clamped_roll =
    modified_roll > 6 ? 6 : modified_roll < 1 ? 1 : modified_roll;
  game.attack.attacker_losses = get_fire_result(
    other_faction(game.attack.attacker),
    base_roll,
    game.attack.defender_table,
    defender_cf,
    defender_shifts,
    clamped_roll,
  );
...
states.eliminate_retreated_units = {
  ...
  piece(p) {
    ...
    if (data.pieces[p].notreplaceable) {
      log(
        `>*${piece_name(p)} in ${space_name(game.location[p])} permanently eliminated`,
      );
...
states.apply_defender_losses = {
  ...
  prompt() {
    ...
    if (game.attack.defender_losses - game.attack.defender_losses_taken > 0) {
      const fort_strength = has_undestroyed_fort(
        game.attack.space,
        active_faction(),
      )
        ? data.spaces[game.attack.space].fort
        : 0;
      loss_options = get_loss_options(
        true,
        game.attack.defender_losses - game.attack.defender_losses_taken,
        get_defenders_pieces(),
        fort_strength,
      );
    }
  ...
  space(s) {
    ...
    if (is_besieged(s)) {
      ...
      game.broken_sieges &&
      set_has(game.broken_sieges, s) &&
      contains_piece_of_faction(s, inactive_faction())
    ) {
      set_delete(game.broken_sieges, s);
      set_control(s, inactive_faction());
    }
    ...
    if (
      has_undestroyed_fort(game.attack.space, game.attack.attacker) &&
      !is_besieged(game.attack.space)
    ) {
    ...
    const flank_attack_active =
      game.attack.is_flank ||
      (game.attack.combat_cards.includes(VON_HUTIER) &&
        events.von_hutier.can_play());
    if (
      !flank_attack_active &&
      is_withdrawal_active() &&
      game.attack.defender_loss_pieces.length > 0
    ) {
...
states.choose_defender_replacement = {
  ...
  piece(p) {
    push_undo();
    replace_defender_unit(
      game.attack.replacement.p,
      game.attack.replacement.s,
      p,
    );
...
function replace_defender_unit(unit, location, replacement) {
  ...
  logi(
    `${piece_name(unit, true)} broke to ${piece_name(replacement)}${log_corps(replacement)}`,
  );
  check_rb_empty(replacement);
}

function army_step_loss_can_be_negated_by_withdrawal(p) {
  if (game.attack.defender_fulfilled_losses_exactly) return true; // if losses were exactly fulfilled, it doesn't matter if the army was permanently eliminated

  if (
    game.attack.ineligible_for_withdrawal &&
    set_has(game.attack.ineligible_for_withdrawal, p)
  )
...
states.withdrawal_negate_step_loss = {
  ...
  prompt() {
    ...
    const has_corps_option = game.attack.defender_loss_pieces.some(
      (p) => data.pieces[p].type === CORPS,
    );
    let can_negate = false;
    game.attack.defender_loss_pieces.forEach((p) => {
      if (
        data.pieces[p].type === CORPS ||
        (!has_corps_option && army_step_loss_can_be_negated_by_withdrawal(p))
      ) {
        can_negate = true;
        gen_action_piece(p);
      }
  ...
  piece(p) {
    ...
    log(
      `${card_name(game.attack.attacker === CP ? WITHDRAWAL_AP : WITHDRAWAL_CP)}\n${piece_name(p)} loss negated`,
    );
    if (set_has(game.removed, p) || is_unit_eliminated(p)) {
      ...
      if (game.attack.defender_replacements[p]) {
        ...
        game.location[replacement] =
          active_faction() === CP ? CP_RESERVE_BOX : AP_RESERVE_BOX;
      }
...
function eliminate_piece(p, force_permanent_elimination, reason) {
  ...
  if (
    force_permanent_elimination ||
    replacement_options.length === 0 ||
    data.pieces[p].notreplaceable ||
    !is_unit_supplied(p)
  ) {
...
function goto_attacker_losses() {
  if (
    game.attack.attacker_losses > 0 &&
    get_loss_options(false, game.attack.attacker_losses, game.attack.pieces, 0)
      .length > 0
  )
    log("Attacker losses:");
...
states.apply_attacker_losses = {
  ...
  prompt() {
    ...
    if (game.attack.attacker_losses - game.attack.attacker_losses_taken > 0)
      loss_options = get_loss_options(
        false,
        game.attack.attacker_losses - game.attack.attacker_losses_taken,
        game.attack.pieces,
        0,
      );
    if (loss_options.length > 0) {
...
function goto_defender_losses() {
  ...
  if (game.attack.defender_losses > 0) {
    const fort_strength = has_undestroyed_fort(
      ...
      ? data.spaces[game.attack.space].fort
      : 0;
    let loss_options = get_loss_options(
      true,
      game.attack.defender_losses,
      get_defenders_pieces(),
      fort_strength,
    );
    if (loss_options.length > 0) log("Defender losses:");
  }

  if (
    game.attack.defender_losses > 0 &&
    get_defenders_pieces().some((p) => set_has(game.retreated, p))
  )
    game.state = "eliminate_retreated_units";
  else game.state = "apply_defender_losses";
}
...
function get_loss_options(is_defender, to_satisfy, units, fort_strength) {
  const is_first_pick =
    (is_defender && game.attack.defender_losses_taken === 0) ||
    (!is_defender && game.attack.attacker_losses_taken === 0);
  ...
  if (!is_defender && is_first_pick) {
    if (units.includes(BEF_ARMY) && to_satisfy >= get_piece_lf(BEF_ARMY))
      return [BEF_ARMY];
    if (units.includes(BEF_CORPS) && to_satisfy >= get_piece_lf(BEF_CORPS))
      return [BEF_CORPS];
    let priority_units = [];
  ...
  if (
    is_defender &&
    is_withdrawal_active() &&
    !game.attack.defender_loss_pieces.some(is_unit_corps)
  ) {
...
function build_loss_tree(parent, valid_paths) {
  ...
  for (let i = 0; i < parent.reduced.length; i++) {
    ...
    if (unit_lf <= parent.to_satisfy) {
      ...
      const full_replacement = find_any_replacement(
        unit,
        node.full_replacements,
      );
      const reduced_replacement = find_any_replacement(
        unit,
        node.reduced_replacements,
      );
  ...
  for (let i = 0; i < parent.options.length; i++) {
    let current_best =
      valid_paths.length === 0 ? parent.to_satisfy : valid_paths[0].to_satisfy;
    let current_best_could_satisfy_additional_loss =
      valid_paths.length === 0
        ? parent.could_satisfy_additional_loss
        : valid_paths[0].could_satisfy_additional_loss;
    let option = parent.options[i];

    // If this option is strictly better than the current best option, then clear out the valid paths.
    if (option.to_satisfy < current_best) valid_paths.length = 0;
    ...
    if (
      option.to_satisfy === current_best &&
      option.could_satisfy_additional_loss &&
      !current_best_could_satisfy_additional_loss
    )
      valid_paths.length = 0;

    // If this option is as good as the current best, add it to the valid paths
    if (option.to_satisfy <= current_best) valid_paths.push(option);
...
function get_replacement_options(unit, available_replacements) {
  ...
  if (unit_data.type !== ARMY) return options;

  if (unit === BEF_ARMY) {
    if (available_replacements.includes(BEF_CORPS)) options.push(BEF_CORPS);
    return options;
  }
  ...
  for (let i = 0; i < available_replacements.length; ++i) {
    ...
    if (replacement_data.type !== CORPS) continue;
    if (
      replacement_data.name === "RU CAVc" ||
      replacement_data.name === "RU Czlc"
    )
      continue; // Russian cavalry corps and Czech Legion cannot be used as replacements
    if (
      (unit_data.nation === BRITAIN && replacement_data.name === "BRc") ||
      (unit_data.nation !== BRITAIN &&
      ...
      else full_options.push(available_replacements[i]);
    }
  }
  ...
  if (full_options.length > 1) {
    ...
    full_options = full_options.filter((p) => {
      if (set_has(names, data.pieces[p].name)) return false;
      else {
...
function is_withdrawal_active() {
  ...
  else return game.attack.combat_cards.includes(WITHDRAWAL_CP);
}
...
function determine_combat_winner() {
  ...
  if (game.attack.defender_losses >= game.attack.attacker_losses) {
    const defender = other_faction(game.attack.attacker);
    to_discard = to_discard.concat(
      game.attack.combat_cards.filter(
        (c) => data.cards[c].faction === defender,
      ),
    );
  }
  if (game.attack.attacker_losses >= game.attack.defender_losses)
    to_discard = to_discard.concat(
      game.attack.combat_cards.filter(
        (c) => data.cards[c].faction === game.attack.attacker,
      ),
    );
  ...
  game.attack.combat_cards.forEach((c) => {
    ...
    if (data.cards[c].remove && !to_discard.includes(c)) to_discard.push(c);
    // MINE_ATTACK, KEMAL, and ROYAL_TANK_CORPS are discarded after use
    if (
      (c === MINE_ATTACK || c === KEMAL || c === ROYAL_TANK_CORPS) &&
      !to_discard.includes(c)
    )
      to_discard.push(c);
  });
  // "They shall not pass" is not discarded when the result is a tie (12.2.11)
  if (
    game.attack.attacker_losses === game.attack.defender_losses &&
    to_discard.includes(THEY_SHALL_NOT_PASS)
  )
  ...
  if (
    game.attack.haig_cancels_ge_retreat &&
  ...
  let intact_unoccupied_fort =
    attacking_unoccupied_fort() && game.attack.defender_losses_taken === 0;
  if (
    (game.attack.defender_losses > game.attack.attacker_losses &&
      attacker_has_full_strength_unit &&
      defender_pieces.length > 0) ||
    was_withdrawal_active
  ) {
    clear_undo();
    set_active_faction(other_faction(game.attack.attacker));
    game.attack.to_retreat = defender_pieces;
    game.attack.retreating_pieces = [];
    if (was_withdrawal_active) game.attack.retreat_length = 1;
    else
      game.attack.retreat_length =
        game.attack.defender_losses - game.attack.attacker_losses === 1 ? 1 : 2;
    game.attack.retreat_paths = [];
    game.attack.to_advance = game.attack.pieces.filter(
      (p) => !is_unit_reduced(p),
    );
    ...
    attacker_has_full_strength_unit &&
    defender_pieces.length === 0 &&
    game.location[game.attack.pieces[0]] !== game.attack.space &&
    !intact_unoccupied_fort
  ) {
    // If the attacker is already in the attack space, it means they just attacked a besieged fort
    clear_undo();
    set_active_faction(game.attack.attacker);
...
function defender_can_cancel_retreat() {
  ...
  const has_uncanceled_trench =
    get_trench_level_for_attack(game.attack.space, defender_faction) > 0 &&
    !game.attack.trenches_canceled;
  if (
    terrain === MOUNTAIN ||
    terrain === SWAMP ||
    ...
    let step_count = 0;
    const pieces = get_pieces_in_space(game.attack.space).filter(
      (p) => data.pieces[p].faction === defender_faction,
    );
    // Cannot remove the last step to cancel a retreat
    for (let p of pieces) {
      if (!is_unit_reduced(p)) return true; // Any full strength unit allows canceling the retreat
...
states.cancel_retreat = {
  ...
  piece(p) {
    ...
    if (is_unit_reduced(p)) {
      ...
      if (
        has_undestroyed_fort(location, game.attack.attacker) &&
        !is_besieged(location)
      ) {
        // Fort is no longer besieged due to losses, but the remaining pieces are allowed to continue occupying the space
        // until the player moves them out or moves another piece in, so we have to remember this space is a "broken siege". (15.2.4)
...
states.choose_retreat_canceling_replacement = {
  ...
  piece(p) {
    push_undo();
    replace_retreat_canceling_unit(
      game.attack.replacement.p,
      game.attack.replacement.s,
      p,
    );
...
function replace_retreat_canceling_unit(unit, location, replacement) {
  game.location[replacement] = location;
  logi(
    `${piece_name(unit, true)} in ${space_name(location)} broke to ${piece_name(replacement)}${log_corps(replacement)}`,
  );
  check_rb_empty(replacement);
}
...
states.defender_retreat = {
  ...
  prompt() {
    ...
    else view.prompt = `Retreat from ${space_name(game.attack.space)}.`;
    ...
    if (game.attack.retreating_pieces.length > 0) {
      ...
      let options = get_retreat_options(
        game.attack.retreating_pieces,
        from,
        game.attack.retreat_length - game.attack.retreat_path.length,
      );
      if (options.length > 0) options.forEach(gen_action_space);
      else gen_action("eliminate");
    } else {
      for (let p of game.attack.to_retreat) gen_action_piece(p);
    }

    if (
      game.attack.retreating_pieces.length + game.attack.to_retreat.length ===
      0
    )
      gen_action("done");
  },
  ...
  piece(p) {
    if (game.attack.retreating_pieces.length === 0) push_undo();
  ...
  eliminate() {
    ...
    for (let p of game.attack.retreating_pieces) {
      let options_for_piece = get_retreat_options(
        [p],
        game.location[p],
        game.attack.retreat_length - game.attack.retreat_path.length,
      );
      if (options_for_piece.length > 0) continue; // Only eliminate pieces that have no valid retreat options
    ...
    if (game.attack.retreating_pieces.length === 0) this._next();
  },
  space(s) {
    update_russian_ne_restriction_flag(
      game.attack.retreating_pieces,
      game.location[game.attack.retreating_pieces[0]],
      s,
    );
    ...
    if (game.attack.retreat_path.length === game.attack.retreat_length) {
      for (let p of game.attack.retreating_pieces) set_add(game.retreated, p);
      this._next();
    }
  },
  _next() {
    if (game.attack.retreat_path.length > 0) {
      const end_space =
        game.attack.retreat_path[game.attack.retreat_path.length - 1];
      if (
        !is_controlled_by(end_space, active_faction()) &&
        !has_undestroyed_fort(end_space, game.attack.attacker)
      ) {
    ...
    if (game.attack.retreating_pieces.length > 0)
      logi(
        piece_list(game.attack.retreating_pieces) +
          " -> " +
          space_list(game.attack.retreat_path),
      );
...
function get_retreat_options(pieces, from, spaces_to_retreat = 1) {
  if (pieces.length === 0 || spaces_to_retreat === 0) return [];

  let faction = other_faction(game.attack.attacker);
  let options = [];

  if (pieces.includes(MONTENEGRIN_CORPS)) return []; // MNC cannot retreat (also prevents play of Withdrawal)

  get_connected_spaces_for_pieces(from, pieces).forEach((conn) => {
    if (conn === from) return;

    if (conn === game.attack.space) return;

    if (spaces_to_retreat === 1 && would_overstack(conn, pieces, faction))
      return;

    if (is_blocked_italian_space(conn, pieces)) return;

    if (contains_piece_of_faction(conn, game.attack.attacker)) return;
    ...
    if (!check_russian_ne_restriction(pieces, conn)) return;

    if (is_brest_litovsk_restricted_group(pieces, conn)) return;

    if (!is_space_at_war(conn)) return;
  ...
  if (spaces_to_retreat === 2) {
    let has_second_space = options.filter(
      (first) => get_retreat_options(pieces, first, 1).length > 0,
    );
    if (has_second_space.length > 0) options = has_second_space;
  }

  // if any options are friendly controlled, remove all enemy-controlled options
  let friendly_options = options.filter(
    (s) => is_controlled_by(s, faction) || is_besieged(s),
  );
  if (friendly_options.length > 0) options = friendly_options;

  // if any spaces are in supply, remove all oos spaces
  let in_supply_options = options.filter((s) =>
    is_every_unit_supplied_in(pieces, s),
  );
  if (in_supply_options.length > 0) options = in_supply_options;
...
states.attacker_advance = {
  ...
  prompt() {
    ...
    const remaining_length =
      game.attack.retreat_length - game.attack.advance_length;

    if (globalThis.RTT_FUZZER) gen_action_done();
    ...
    if (game.attack.advance_length === 0) {
      ...
      let pieces_in_space_not_advancing = get_pieces_in_space(
        current_space,
      ).filter((p) => !game.attack.advancing_pieces.includes(p));
      if (pieces_in_space_not_advancing.length < STACKING_LIMIT)
        for (let p of game.attack.advancing_pieces) gen_action_piece(p);
    ...
    if (game.attack.advancing_pieces.length > 0) {
      if (spaces.length > 0) spaces.forEach(gen_action_space);
      else gen_action("stop");
    }
  },
  piece(p) {
    if (set_has(game.attack.advancing_pieces, p)) {
      ...
      if (
        has_undestroyed_fort(end_space, other_faction(game.attack.attacker))
      ) {
  ...
  stop() {
    ...
    logi(
      piece_list(game.attack.advancing_pieces) + " -> " + space_name(end_space),
    );
...
function get_possible_advance_spaces(pieces) {
  if (game.attack.advance_length >= game.attack.retreat_length) return [];
  ...
  if (game.attack.space !== location_of_advancing_units) {
    if (
      game.attack.retreat_length > 1 &&
      can_advance_through(game.attack.space, pieces, game.attack.retreat_paths)
    )
  ...
  if (is_besieged(location_of_advancing_units)) {
    if (
      !can_besiege(
        location_of_advancing_units,
        get_pieces_in_space(location_of_advancing_units).filter(
          (p) => !pieces.includes(p),
        ),
      )
    ) {
  ...
  if (!terrain_allows_advance) return [];

  let spaces = [];
  for (let path of game.attack.retreat_paths) {
    if (can_advance_into(path[0], pieces)) set_add(spaces, path[0]); // Add the first retreat space from each retreated path
  }
  ...
  if (
    ...
    if (game.attack.space !== AMIENS) set_delete(spaces, AMIENS);
    if (game.attack.space !== CALAIS) set_delete(spaces, CALAIS);
    if (game.attack.space !== OSTEND) set_delete(spaces, OSTEND);
  }
  ...
  let connected = get_connected_spaces_for_pieces(
    location_of_advancing_units,
    pieces,
  );
...
function can_advance_into(space, units, can_overstack = false) {
  ...
  if (
    has_undestroyed_fort(space, other_faction(game.attack.attacker)) &&
    !is_besieged(space) &&
    !can_besiege(space, units)
  )
    return false;
  ...
  if (!is_space_at_war(space)) return false;

  if (!can_overstack && would_overstack(space, units, game.attack.attacker))
    return false;

  if (is_blocked_italian_space(space, units)) return false;

  return true;
}

function can_advance_through(space, units, retreat_paths) {
  if (!can_advance_into(space, units, true)) return false;
...
function eliminate_ru_units_violating_treaty_of_brest_litovsk() {
  if (game.events.treaty_of_brest_litovsk === 0) return;
  ...
  log(
    "RU units can only operate in Russia, Germany, Turkey, Austria and Romania",
  );

  const allowed_nations = [RUSSIA, GERMANY, TURKEY, AUSTRIA_HUNGARY, ROMANIA];
  for (let p of all_pieces_by_nation[RUSSIA]) {
    const location = game.location[p];
    if (location >= AP_RESERVE_BOX || location === 0) continue;
...
function has_undestroyed_fort(space, faction) {
  ...
  return (
    space_data.fort !== undefined &&
    space_data.fort > 0 &&
    space_data.faction === faction &&
    !set_has(game.forts.destroyed, space)
  );
}
...
function get_sud_army_activation_cost(pieces) {
  if (!is_possible_sud_army_stack(pieces)) return undefined;
  ...
  pieces.forEach((p) => {
    const nation = data.pieces[p].nation;
    if (nation === GERMANY) ge++;
    else if (nation === AUSTRIA_HUNGARY) ah++;
    else other++;
  });
  if (ah === 1 && ge >= 1 && other === 0) return 1;
  else return 2;
}
...
function cost_to_activate(space, type) {
  ...
  if (
    spaces_where_belgian_units_treated_as_british.includes(space) &&
  ...
  if (
    active_faction() === CP &&
    game.events.sud_army > 0 &&
    is_possible_sud_army_stack(pieces)
  ) {
  ...
  if (
    active_faction() === CP &&
    game.events.moltke > 0 &&
    !game.events.falkenhayn
  ) {
  ...
  let fall_of_the_tsar_attack =
    game.events.fall_of_the_tsar > 0 && num_russians > 0 && type === ATTACK;
  ...
  if (
    faction === AP &&
    set_has(nations, BRITAIN) &&
    is_space_supplied_through_mef(space)
  ) {
    cost = 0;
    pieces.forEach((p) => {
      if (p === MEF_ARMY) {
...
function goto_attrition_phase() {
  ...
  for (let s = 1; s < map_space_count; ++s) {
    ...
    if (!is_space_at_war(s)) continue;
    if (s >= AP_RESERVE_BOX) continue;
    if (is_mef_space(s)) {
      continue; // MEF spaces do not flip control
    }
    if (s === ARABIA_SPACE) continue;
    if (s === MEDINA && turkish_units_in_medina) continue;
    ...
    if (controlling_faction === CP && game.location[TURKISH_SN_CORPS] === s)
      // SN corps keeps its space safe from attrition per 11.1.16
      continue;
  ...
  if (
    game.attrition.ap.spaces.length > 0 ||
    game.attrition.ap.pieces.length > 0
  ) {
    game.state = "attrition_phase";
    set_active_faction(AP);
  } else if (
    game.attrition.cp.spaces.length > 0 ||
    game.attrition.cp.pieces.length > 0
  ) {
    game.state = "attrition_phase";
...
states.attrition_phase = {
  ...
  prompt() {
    ...
    const pieces_str =
      pieces.length <= 3 && pieces.length > 0 ? ` (${piece_list(pieces)})` : "";
    const spaces_str =
      spaces.length <= 3 && spaces.length > 0 ? ` (${space_list(spaces)})` : "";
  ...
  piece(p) {
    ...
    if (data.pieces[p].type === ARMY) {
      log_alert(
        `Permanently removed ${piece_name(p)} from ${space_name(game.location[p])}`,
      );
      game.location[p] = PERM_ELIMINATED_BOX;
      set_add(game.removed, p);
    } else {
    ...
    if (
      game.attrition[active_faction()].spaces.length === 0 &&
      game.attrition[active_faction()].pieces.length === 0
    ) {
      if (
        game.attrition[other_faction(active_faction())].spaces.length > 0 ||
        game.attrition[other_faction(active_faction())].pieces.length > 0
      ) {
        clear_undo();
        switch_active_faction();
      } else {
...
function goto_siege_phase() {
  if (game.forts.besieged.length > 0) {
    ...
    const ap_has_sieges = game.forts.besieged.some(
      (s) => data.spaces[s].faction === CP,
    );
...
states.siege_phase = {
  ...
  prompt() {
    let siege_spaces = game.sieges_to_roll.filter(
      (s) => data.spaces[s].faction === inactive_faction(),
    );
  ...
  space(s) {
    ...
    log(
      `Siege at ${space_name(s)} (${data.spaces[s].fort} CF): ${fmt_roll(roll, drm)}`,
    );
    ...
    if (game.sieges_to_roll.length === 0) {
      ...
      !game.sieges_to_roll.some(
        (s) => data.spaces[s].faction === inactive_faction(),
      ) &&
      game.sieges_to_roll.some(
        (s) => data.spaces[s].faction === active_faction(),
      )
    ) {
      clear_undo();
      switch_active_faction();
...
function goto_war_status_phase() {
  ...
  if (game.cp.mo !== NONE) {
    ...
    log(
      `-1 VP -- ${faction_name(CP)} failed to conduct their mandated offensive (${nation_name(game.cp.mo)})`,
    );
  }
  ...
  if (!nation_at_war(ITALY) && game.ap.commitment === COMMITMENT_TOTAL) {
    ...
    log(
      `+1 VP -- ${nation_name(ITALY)} is still neutral but ${faction_name(AP)} at Total War`,
    );
  }

  const french_mutiny_active =
    game.events.french_mutiny > 0 && game.ap.mo === FRANCE;
  ...
  if (game.ap.mo !== NONE && !french_mutiny_active) {
    ...
    log(
      `+1 VP -- ${faction_name(AP)} failed to conduct their mandated offensive (${nation_name(game.ap.mo)})`,
    );
  }
  ...
  if (game.turn !== 1) {
    if (game.ap.ws >= 4 && game.ap.commitment === COMMITMENT_MOBILIZATION) {
      ...
      log_h3(
        `${faction_name(AP)} War Commitment Level rises to Limited War`,
        AP,
      );
    ...
    if (game.cp.ws >= 4 && game.cp.commitment === COMMITMENT_MOBILIZATION) {
      ...
      log_h3(
        `${faction_name(CP)} War Commitment Level rises to Limited War`,
        CP,
      );
...
function add_cards_to_deck(faction, commitment, deck) {
  for (let i = 1; i < data.cards.length; i++) {
    if (
      data.cards[i].commitment === commitment &&
      data.cards[i].faction === faction &&
      data.utils.is_card_allowed_to_deal(i, game.options)
    ) {
      deck.push(i);
    }
  ...
  if (use_historical_scenario_rules()) {
    ...
    [USA_REINFORCEMENTS_2, USA_REINFORCEMENTS_3].forEach((c) => {
      if (
        ...
        !game.events.reinforcements.includes(c)
      ) {
...
function get_result_message(prefix, result) {
  if (result === AP) return `${prefix}${faction_name(AP)} won.`;
  if (result === CP) return `${prefix}${faction_name(CP)} won.`;
  if (result === "Draw") return `${prefix}Game ended in a draw.`;
...
function apply_replacement_phase_events() {
  ...
  if (
    game.cp.commitment === COMMITMENT_TOTAL &&
    is_controlled_by(SEDAN, CP) &&
    !game.cp.first_total_war
  ) {
    let cp_fr_be_spaces = 0;
    cp_fr_be_spaces += all_spaces_by_nation[FRANCE].filter((s) =>
      is_controlled_by(s, CP),
    ).length;
    cp_fr_be_spaces += all_spaces_by_nation[BELGIUM].filter((s) =>
      is_controlled_by(s, CP),
    ).length;
    if (cp_fr_be_spaces >= 3) {
      game.rp.ge++;
      log(
        `${faction_name(CP)} gains 1 German RP for controlling Sedan and 2 other French/Belgian spaces`,
      );
    }
  } else {
...
function summarize_rps(faction) {
  ...
  if (faction === AP) {
    ...
    if (game.rp.allied > 0)
      summary.push(`${rp_string(game.rp.allied, "Allied")}`);
...
states.replacement_phase = {
  ...
  prompt() {
    if (game.army_to_rebuild) {
      ...
      get_army_replacement_spaces(game.army_to_rebuild).forEach(
        gen_action_space,
      );
    } else {
  ...
  piece(p) {
    ...
    if (piece_data.type === ARMY) {
      if (is_unit_eliminated(p)) {
        ...
        set_delete(game.reduced, p);
        log(
          `Flipped ${piece_name(p)} in ${space_name(game.location[p])} to full strength`,
        );
      ...
      if (is_unit_eliminated(p)) {
        set_add(game.reduced, p);
        if (p === BRITISH_ANA_CORPS) game.location[p] = ARABIA_SPACE;
        else
          game.location[p] =
            active_faction() === AP ? AP_RESERVE_BOX : CP_RESERVE_BOX;
        ...
        spend_rps(get_rp_type(p), 0.5);
        log(
          `Flipped ${piece_name(p)}${log_corps(p)} in ${space_name(game.location[p])} to full strength`,
        );
      }
    }
  },
...
function get_replaceable_units() {
  ...
  for (let i = 1; i < data.pieces.length; ++i) {
    ...
    if (piece_data.faction !== active_faction()) continue;

    if (piece_data.notreplaceable) continue;

    const rp_type = get_rp_type(i);
    const rps_available = get_rps_of_type(rp_type);
    if (rps_available === 0) continue;
    ...
    if (is_controlled_by(WARSAW, AP) && piece_data.name === "PLc") continue;

    if (piece_data.type === ARMY) {
      if (
        (s === AP_ELIMINATED_BOX || s === CP_ELIMINATED_BOX) &&
        get_army_replacement_spaces(i).length === 0
      )
        continue;
      if (rps_available < 1) continue;
    }
    ...
    if (is_unit_eliminated(i)) {
      ...
      if (s === AP_RESERVE_BOX || s === CP_RESERVE_BOX) {
        ...
        // TODO: 17.1.4.1 check
        if (is_unit_supplied_for_rp_in(i, s)) units.push(i);
      }
    }
  }
...
function get_army_replacement_spaces(p) {
  ...
  if (data.pieces[p].nation === BELGIUM) {
    ...
    if (spaces.length === 0) {
      if (
        is_controlled_by(CALAIS, AP) &&
        is_unit_supplied_in(p, CALAIS) &&
        !is_fully_stacked(CALAIS, AP)
      )
  ...
  if (data.pieces[p].nation === SERBIA) {
    ...
    if (game.events.salonika > 0 || game.events.greece_entry > 0) {
      if (
        is_controlled_by(SALONIKA, AP) &&
        is_unit_supplied_in(p, SALONIKA) &&
        !is_fully_stacked(SALONIKA, AP)
      )
        set_add(spaces, SALONIKA);
    }

    // Exception: Serb armies may not be recreated at Belgrade if Nis is under CP control.
    if (is_controlled_by(NIS, CP)) set_delete(spaces, BELGRADE);
  }
...
function get_connected_spaces_for_pieces(s, pieces) {
  if (
    pieces.length > 0 &&
    pieces.every((p) => data.pieces[p].nation === data.pieces[pieces[0]].nation)
  ) {
...
function get_connected_spaces(s, nation) {
  if (nation === undefined) return data.spaces[s].connections;
  return (
    data.spaces[s].limited_connections[nation] ?? data.spaces[s].connections
  );
}
...
function is_port(s, faction, is_supply_check = false) {
  ...
  if (
    s === CONSTANTINOPLE &&
    faction === AP &&
    !is_controlled_by(GALLIPOLI, AP)
  )
    return false; // AP cannot use Constantinople as a port unless they control Gallipoli

  return (
    (faction === AP && data.spaces[s].apport) ||
    (faction === CP && data.spaces[s].cpport)
  );
}
...
function get_supply_mask(source) {
  switch (source) {
    case ESSEN:
      return SUPPLY_MASK.Essen;
    case BRESLAU:
      return SUPPLY_MASK.Breslau;
    case SOFIA:
      return SUPPLY_MASK.Sofia;
    case CONSTANTINOPLE:
      return SUPPLY_MASK.Constantinople;
    case PETROGRAD:
      return SUPPLY_MASK.Petrograd;
    case MOSCOW:
      return SUPPLY_MASK.Moscow;
    case KHARKOV:
      return SUPPLY_MASK.Kharkov;
    case CAUCASUS:
      return SUPPLY_MASK.Caucasus;
    case BELGRADE:
      return SUPPLY_MASK.Belgrade;
    case LONDON:
      return SUPPLY_MASK.London;
    case SALONIKA:
      return SUPPLY_MASK.Salonika;
    case BASRA:
      return SUPPLY_MASK.Basra;
  }
...
function fill_supply_cache(faction, cache, sources, options) {
  ...
  for (let s = 1; s < map_space_count; ++s) {
    if (!is_space_at_war(s)) blocked_spaces[s] = 1;
  }
...
function update_supply() {
  ...
  if (nation_at_war(BULGARIA)) cp_sources.push(SOFIA);
  if (nation_at_war(TURKEY)) cp_sources.push(CONSTANTINOPLE);
  ...
  fill_supply_cache(CP, game.supply, cp_sources, {
    use_ports: true,
    national_connections: TURKEY,
  }); // Not all CP supply can follow Turkish connections, but this only applied to Medina, so it's easier to add a special check for non-Turkish units in Medina
  fill_supply_cache(
    AP,
    game.supply,
    [PETROGRAD, MOSCOW, KHARKOV, CAUCASUS, BELGRADE],
    { national_connections: RUSSIA },
  );
  fill_supply_cache(AP, game.supply, [LONDON], {
    use_ports: true,
    set_nonitalian_path: true,
    set_nonmef_path: true,
  });

  // Special mask applied when searching spaces using Italian connections
  fill_supply_cache(AP, game.supply, [LONDON], {
    use_ports: true,
    national_connections: ITALY,
    override_mask: SUPPLY_MASK.London_Italian,
  });
  ...
  fill_supply_cache(AP, game.supply, [BASRA], {
    national_connections: BRITAIN,
  });
...
function is_unit_supplied(p) {
  ...
  if (location === 0 || location === PERM_ELIMINATED_BOX) return true;
  ...
  if (is_controlled_by(opp)) {
    if (has_undestroyed_fort(s, other_faction(faction))) {
      for (let p of all_pieces_by_faction[faction])
        if (game.location[p] === s) return false;
    }
    return true;
  }
  return false;
...
function is_unit_supplied_in(p, s, for_rp = false) {
  ...
  if (nation === GREECE && !nation_at_war(GREECE))
    // Limited Greek entry
    return true;
  ...
  if (nation === SERBIA) {
    if (data.spaces[s].nation === SERBIA) return is_supply_not_blocked(s, AP); // Serbian units are always in supply in Serbia
    if (check_supply_cache(game.supply, s, [SALONIKA])) return true; // Serbian units can trace supply to Salonika if it is friendly controlled
  }
  ...
  if (nation === ITALY)
    return (
      check_supply_cache(
        ...
        s,
        [LONDON],
        SUPPLY_MASK.London_Italian,
      ) && check_supply_cache(game.supply, s, [], SUPPLY_MASK.NonMEFPath)
    );

  if (
    data.pieces[p].faction === AP &&
    is_space_supplied_through_mef(s) &&
    piece_requires_london_supply(p)
  ) {
  ...
  if (for_rp)
    return check_supply_cache(
      game.supply,
      s,
      get_supply_sources_for_piece_rp(p),
    );
...
function is_space_supplied(s, faction) {
  if (faction === CP) {
    return check_supply_cache(game.supply, s, [
      ESSEN,
      BRESLAU,
      SOFIA,
      CONSTANTINOPLE,
    ...
    if (
      data.spaces[s].nation === ALBANIA &&
      (!is_space_at_war(TARANTO) || is_space_supplied(TARANTO, AP))
    ) {
    ...
    if (s === MEDINA) {
      ...
      return (
        is_space_supplied(JERUSALEM, AP) ||
        is_space_supplied(AMMAN, AP) ||
        is_space_supplied(AQABA, AP)
      );
    }
    return check_supply_cache(
      game.supply,
      s,
      [LONDON, PETROGRAD, MOSCOW, KHARKOV, CAUCASUS, BELGRADE, SALONIKA],
      SUPPLY_MASK.London_Italian,
    );
  }
}

const all_supply_sources = [
  ESSEN,
  BRESLAU,
  SOFIA,
  CONSTANTINOPLE,
...
function check_supply_cache(
  cache,
  location,
  sources = all_supply_sources,
  additional_mask,
) {
  let source_mask = 0;

  if (additional_mask) source_mask |= additional_mask;

  for (let source of sources) source_mask |= get_supply_mask(source);
...
function get_supply_sources_for_piece(p) {
  ...
  if (data.pieces[p].faction === AP) return [LONDON];
...
function is_unit_supplied_through_italy(p) {
  ...
  let supplied_from_london =
    game.supply[game.location[p]] & SUPPLY_MASK.London_Italian;
  let has_only_italian_path = !(
    game.supply[game.location[p]] & SUPPLY_MASK.NonItalianPath
  );
  return supplied_from_london && has_only_italian_path;
}

function is_space_supplied_through_mef(s) {
  if (!is_space_supplied(s, AP)) return false;
...
function is_space_supplied_or_has_supplied_unit(s, faction) {
  if (is_space_supplied(s, faction)) return true;
  for (let p of all_pieces_by_faction[faction])
    if (game.location[p] === s) if (is_unit_supplied_in(p, s)) return true;
  return false;
}
...
function query_supply(sources) {
  let mask = 0;
  for (let source of sources) {
    if (source === LONDON) mask |= SUPPLY_MASK.London_Italian;
    else mask |= get_supply_mask(source);
  }
  let spaces = [0];
...
function update_oos_pieces() {
  ...
  for (let p = 1; p < data.pieces.length; ++p) {
    if (!nation_at_war(data.pieces[p].nation)) continue;
    if (
      game.location[p] !== 0 &&
      game.location[p] < map_space_count &&
      !is_unit_supplied(p) &&
      !set_has(moving_pieces, p)
    ) {
...
events.cp_severe_weather = {
  can_play() {
    ...
    if (
      terrain === MOUNTAIN &&
      (season === SEASON_FALL || season === SEASON_WINTER)
    )
      return true;

    return (
      terrain === SWAMP && (season === SEASON_FALL || season === SEASON_SPRING)
    );
  },
  can_apply() {
    return this.can_play();
...
function is_unit_eligible_for_landwehr(p) {
  if (data.pieces[p].nation !== GERMANY) return false;
  if (!is_unit_reduced(p)) return false;
...
states.landwehr = {
  ...
  prompt() {
    let spent_rp = game.landwehr_pieces.reduce((acc, p) => {
      return acc + (data.pieces[p].type === CORPS ? 0.5 : 1);
    }, 0);
    if (spent_rp >= 2) {
      view.prompt = `Landwehr: Done.`;
      view.actions.end_action = 1;
    } else {
      let pieces = all_pieces_by_nation[GERMANY].filter(
        is_unit_eligible_for_landwehr,
      );
  ...
  piece(p) {
    ...
    set_delete(game.reduced, p);
    log(
      `Flipped ${piece_name(p)} (${space_name(game.location[p])}) to full strength`,
    );
...
events.reichstag_truce = {
  can_play() {
    if ((game.turn === 1) & (game.cp.actions.length > 0)) {
      if (game.cp.commitment !== COMMITMENT_TOTAL) return true;
    } else {
      if (game.cp.commitment !== COMMITMENT_TOTAL) return true;
    }
    return false;
...
events.liman_von_sanders = {
  ...
  apply() {
    ...
    if (game.attack.attacker === CP) game.attack.attacker_drm += 1;
    else game.attack.defender_drm += 1;
  },
};
...
events.mata_hari = {
  ...
  play() {
    clear_undo();
    log(
      `${card_name(MATA_HARI)} reveals the contents of the ${faction_name(AP)} hand:`,
    );
...
events.flamethrowers = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker !== CP) return false;
...
events.place_of_execution = {
  can_play() {
    if (all_french_forts_destroyed()) return true;
    return false;
  },
  can_apply() {
    if (!game.events.falkenhayn) return false;

    if (game.events.h_l_take_command > 0) return false;

    if (game.attack.attacker !== CP) return false;

    const space_data = data.spaces[game.attack.space];
    return (
      space_data.nation === FRANCE &&
      space_data.fort > 0 &&
      !game.forts.destroyed.includes(game.attack.space)
    );
  },
  apply() {
...
events.kemal = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker !== AP) return false;
    return get_defenders_pieces().some(
      (p) => data.pieces[p].nation === TURKEY && get_piece_cf(p) > 0,
    );
  },
  can_apply() {
    if (!game.attack) return false;
...
states.war_in_africa = {
  ...
  prompt() {
    if (game.war_in_africa_removed === undefined) {
      ...
      view.prompt =
        "War in Africa: Remove a British Corps or pass to lose 1 VP.";
      let full_strength_piece = false;
      let pieces = [];
      for (let p = 1; p < data.pieces.length; ++p) {
        if (
          data.pieces[p].nation === BRITAIN &&
          data.pieces[p].type === CORPS &&
          game.location[p] !== 0 &&
          game.location[p] !== AP_ELIMINATED_BOX &&
          ...
          if (!is_unit_reduced(p)) full_strength_piece = true;
          pieces.push(p);
        }
      }
...
events.von_below = {
  can_play() {
    ...
    if (defenders.length === 0) return false;
    for (let d of defenders) {
      if (data.pieces[d].nation !== ITALY) return false;
    }
...
events.von_hutier = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker !== CP) return false;
    if (!contains_piece_of_nation(game.attack.space, RUSSIA)) return false;
    return game.attack.pieces.some((p) => data.pieces[p].nation === GERMANY);
  },
  play() {
...
events.michael = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker !== CP) return false;
    if (!game.events.h_l_take_command) return false;
    return game.attack.pieces.some((p) => data.pieces[p].nation === GERMANY);
  },
  can_apply() {
    return this.can_play();
...
events.fall_of_the_tsar = {
  can_play() {
    return (
      game.events.tsar_takes_command > 0 &&
      game.cp.ru_vp + game.ap.ws + game.cp.ws >= 33
    );
  },
  play() {
...
events.bolshevik_revolution = {
  can_play() {
    ...
    return (
      game.events.fall_of_the_tsar > 0 &&
      game.turn > game.events.fall_of_the_tsar &&
      (game.cp.ru_vp > game.tsar_fell_cp_russian_vp ||
        cp_controls_all_russian_vp_spaces)
    );
  },
  play() {
    game.events.bolshevik_revolution = game.turn;
    if (game.ap.mo === RUSSIA || game.ap.mo === RU_GE || game.ap.mo === RU_AH) {
      game.ap.mo = NONE;
    }
...
events.lloyd_george = {
  can_play() {
    return !(
      game.events.michael > 0 ||
      game.events.blucher > 0 ||
      game.events.peace_offensive > 0
    );
  },
  play() {
...
function is_lloyd_george_active() {
  return (
    game.events.lloyd_george === game.turn && !game.events.lloyd_george_canceled
  );
}
...
events.kaisertreu = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker === CP)
      return game.attack.pieces.some(
        (p) => data.pieces[p].nation === AUSTRIA_HUNGARY,
      );
    else
      return get_defenders_pieces().some(
        (p) => data.pieces[p].nation === AUSTRIA_HUNGARY,
...
events.turk_determination = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker !== AP) return false;
    if (!contains_piece_of_nation(game.attack.space, TURKEY)) return false;
...
events.achtung_panzer = {
  can_play() {
    ...
    return (
      undefined === data.spaces[game.attack.space].terrain &&
      game.attack.pieces.some((p) => data.pieces[p].nation === GERMANY)
    );
  },
  can_apply() {
...
events.alberich = {
  can_play() {
    ...
    if (game.attack.attacker !== AP) return false;
    if (
      game.attack.combat_cards.includes(ROYAL_TANK_CORPS) ||
      game.action_state.yanks_and_tanks
    )
      return false;
...
events.putnik = {
  ...
  apply() {
    ...
    if (game.attack.attacker === AP) game.attack.attacker_drm += 1;
    else game.attack.defender_drm += 1;
  },
};
...
events.rape_of_belgium = {
  can_play() {
    return (
      game.events.guns_of_august &&
      game.ap.commitment === COMMITMENT_MOBILIZATION
    );
  },
  play() {
...
events.ap_air_superiority = {
  can_play() {
    ...
    for (let p of game.attack.pieces) {
      if (
        data.pieces[p].nation === FRANCE ||
        (data.pieces[p].nation === BRITAIN && !is_minor_british_nation(p))
      )
        return true;
    }
    return false;
...
states.great_retreat_option = {
  ...
  prompt() {
    ...
    gen_action_pass();
    get_defenders_pieces()
      .filter((p) => data.pieces[p].nation === RUSSIA)
      .forEach(gen_action_piece);
  },
  piece(p) {
    push_undo();
    log("Great Retreat:");
...
states.great_retreat = {
  ...
  prompt() {
    if (game.attack.great_retreat !== 0) {
      ...
      let pieces_to_retreat = get_defenders_pieces().filter(
        (p) => data.pieces[p].nation === RUSSIA,
      );
  ...
  space(s) {
    ...
    if (set_has(game.broken_sieges, from)) set_delete(game.broken_sieges, from);

    game.location[game.attack.great_retreat] = s;
    if (
      !is_controlled_by(s, AP) &&
      !has_undestroyed_fort(s, CP) &&
      can_take_control([game.attack.great_retreat])
    )
      set_control(s, AP);
    update_siege(game.attack.space);
    update_siege(s);
  ...
  eliminate() {
    ...
    if (
      has_undestroyed_fort(game.attack.space, AP) ||
      ...
      set_active_faction(game.attack.attacker);
      let full_strength_attackers = game.attack.pieces.filter(
        (p) => !is_unit_reduced(p),
      );
...
events.salonika = {
  can_play() {
    return (
      !game.events.greece_entry ||
      (is_controlled_by(SALONIKA, AP) && !is_fully_stacked(SALONIKA, AP))
    );
  },
  play() {
    ...
    if (!is_fully_stacked(SALONIKA, AP) && game.salonika_sr_remaining > 0) {
      for (let p = 1; p < data.pieces.length; ++p) {
        ...
        if (
          ...
          !is_bef_unit(p) &&
          (nation === FRANCE ||
            (nation === BRITAIN && !is_minor_british_nation(p))) &&
...
events.they_shall_not_pass = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker !== CP) return false;
    if (data.spaces[game.attack.space].nation !== FRANCE) return false;
    if (!has_undestroyed_fort(game.attack.space, AP)) return false;
...
events.kerensky_offensive = {
  can_play() {
    return (
      game.events.fall_of_the_tsar > 0 && !game.events.bolshevik_revolution > 0
    );
  },
  play() {
  ...
  can_apply() {
    if (!game.attack.pieces.some((p) => data.pieces[p].nation === RUSSIA))
      return false;
    if (
      !get_defenders_pieces().some(
        (p) =>
          data.pieces[p].nation === AUSTRIA_HUNGARY ||
          data.pieces[p].nation === BULGARIA ||
          data.pieces[p].nation === TURKEY,
      )
    )
      return false;
    return true;
...
events.royal_tank_corps = {
  can_play() {
    if (!game.attack) return false;
    if (game.attack.attacker !== AP) return false;
    if (!game.events.landships && !game.options.valiant) return false;
...
events.over_there = {
  can_play() {
    return (
      game.events.zimmermann_telegram > 0 &&
      game.turn > game.events.zimmermann_telegram
    );
  },
  play() {
  ...
  can_play() {
    ...
    for (let p of all_pieces_by_nation[FRANCE])
      if (
        data.pieces[p].type === ARMY &&
        is_unit_reduced(p) &&
        spaces.includes(game.location[p])
      )
        return true;
    return false;
  },
...
states.paris_taxis = {
  ...
  prompt() {
    view.prompt =
      "Paris Taxis: Flip a reduced French army in or adjacent to Paris.";
    const spaces = [PARIS, AMIENS, ROUEN, CHATEAU_THIERRY, ORLEANS, MELUN];
    for (let p of all_pieces_by_nation[FRANCE])
      if (
        data.pieces[p].type === ARMY &&
...
events.alpine_troops = {
  can_play() {
    ...
    return !!(
      game.attack.attacker === AP &&
      game.attack.pieces.every((p) => data.pieces[p].nation === ITALY)
    );
  },
  can_apply() {
    return this.can_play();
...
events.czech_legion = {
  ...
  play() {
    for (let p = 1; p < data.pieces.length; ++p) {
      const piece_data = data.pieces[p];
      if (
        is_unit_eliminated(p) &&
        piece_data.nation === AUSTRIA_HUNGARY &&
        piece_data.type === CORPS
      ) {
        game.location[p] = PERM_ELIMINATED_BOX;
        set_add(game.removed, p);
...
events.maude = {
  can_play() {
    if (!game.attack) return false;
    let attacking_british_pieces = game.attack.pieces.filter(
      (p) => data.pieces[p].nation === BRITAIN,
    );
    if (attacking_british_pieces.length === 0) return false;
...
function can_trace_ap_supply_through_basra(list) {
  ...
  if (list.some((p) => !is_unit_supplied(p))) return false;

  // Check if Basra is supplied
  if (!is_space_supplied(BASRA, AP)) return false;
  ...
  while (frontier.length > 0 && !found_basra) {
    ...
    for (let conn of connected) {
      if (set_has(visited, conn)) continue;
      if (!is_controlled_by(conn, AP) && !is_besieged(conn)) continue;
      if (conn === BASRA) {
        found_basra = true;
        break;
      }
...
events.backs_to_the_wall = {
  can_play() {
    ...
    if (![FRANCE, BELGIUM].includes(data.spaces[game.attack.space].nation))
      return false;
    return get_defenders_pieces().some(
      (p) => data.pieces[p].nation === BRITAIN && data.pieces[p].type === ARMY,
    );
  },
  can_apply() {
    return this.can_play();
...
function array_remove_item(array, item) {
  ...
  if (i >= 0) array_remove(array, i);
}

function array_delete_pair(array, index) {
  var i,
    n = array.length;
  for (i = index + 2; i < n; ++i) array[i - 2] = array[i];
  array.length = n - 2;
}
...
function push_undo() {
  ...
  for (let k in game) {
    let v = game[k];
    if (k === "undo") continue;
    else if (k === "rollback") continue;
    else if (k === "rollback_state") continue;
...
function pop_all_undo() {
  if (game.undo.length > 1) game.undo.length = 1;
  pop_undo();
}
...
function base64_encode(binary) {
  if (Uint8Array.prototype.toBase64) return binary.toBase64();
  return Buffer.from(binary).toString("base64");
}

function base64_decode(ascii) {
  if (Uint8Array.fromBase64) return Uint8Array.fromBase64(ascii);
...
function decompress_state(state_str) {
  if (typeof state_str === "string")
    return JSON.parse(
      textdec.decode(lz4.uncompressBlob(base64_decode(state_str))),
    );
  return state_str;
}
...
function save_rollback_point() {
  ...
  const is_turn_start =
    game.ap.actions.length === 0 && game.cp.actions.length === 0;
  ...
  if (is_turn_start) {
    ...
    const count_action_rounds = game.rollback.filter(
      (r) => !r.turn_start,
    ).length;
...
states.review_rollback_proposal = {
  ...
  accept() {
    ...
    if (phase === "Action Phase") {
      ...
      let ar_faction =
        game.ap.actions.length === game.cp.actions.length ? AP : CP;
      ...
      let round =
        ar_faction === AP ? game.ap.actions.length : game.cp.actions.length;
...
function log_h3(msg, faction) {
  ...
  log_br();
  if (faction === AP) log(".h3ap " + msg);
  else if (faction === CP) log(".h3cp " + msg);
  else log(".h3 " + msg);
  log_br();
}
...
function log_attack(msg) {
  log_br();
  if (active_faction() === AP) log("#ap " + msg);
  else log("#cp " + msg);
}
...
function log_piece_move(p) {
  ...
  else logi(piece_name(p) + " -> " + space_name(game.location[p]));
}

function die_color(faction) {
  if (faction === AP) return "W";
  else return "B";
}
...
function fmt_roll(roll, drm, faction) {
  ...
  if (drm !== undefined && drm !== 0) {
    if (drm > 0) s = s + ` + ${drm} = ${roll + drm}`;
    else s = s + ` - ${-drm} = ${roll + drm}`;
  }
  return s;
}
...
function assert_stacking_limits() {
  const rule_text =
    "10.1.2 Stacking limits are in effect at all times except during SR and movement";
  ...
  for (const faction of [AP, CP]) {
    for (let s = 1; s < map_space_count; ++s) {
      if (is_overstacked(s, faction)) {
        throw new Error(
          `Rule violation by ${faction} in S${s} ${space_name(s)}: ${rule_text}`,
        );
      }
    }
  }
...
function assert_opposing_sides_not_stacked() {
  const rule_text =
    "10.1.5 Units of opposing sides may never be stacked together. (unless besieged)";

  for (let s = 1; s < map_space_count; ++s) {
    if (is_besieged(s)) continue;
    let has_ap = false;
    let has_cp = false;
    for (const p of get_pieces_in_space(s)) {
      const faction = data.pieces[p].faction;
      if (faction === AP) has_ap = true;
      else if (faction === CP) has_cp = true;
    }
    if (has_ap && has_cp) {
...
function assert_trench_level() {
  const rule_text =
    "11.2.3 [...] Trench levels may never be more than Level 2.";
...
function assert_reinforcement_rules() {
  ...
  if (game.state !== "place_reinforcements") return;

  if (!game.reinforcements.length) return;

  const first_piece = game.reinforcements[0];
  const first_piece_data = data.pieces[first_piece];
  if (first_piece_data.type !== ARMY) return;

  const spaces = get_available_reinforcement_spaces(first_piece);
  spaces.forEach((s) => {
    if (first_piece_data.name === "BR MEF" && is_mef_space(s)) return;
    if (!is_space_supplied(s, active_faction())) {
      throw new Error(
        `Rule violation by allowing reinforcement of ${piece_name(first_piece)} in S${s} ${space_name(s)}: ${rule_text}`,
      );
    }
  });
}
...
function assert_italian_operation() {
  const rule_text =
    "5.7.4 part 6, until AP is at Total War only Austrian and Italian armies may operate in Italy";

  if (game.ap.commitment === COMMITMENT_TOTAL) return;
  ...
  for (let p = 1; p < data.pieces.length; ++p) {
    ...
    if (
      ...
      throw new Error(
        `Rule violation by ${piece_name(p)} in S${game.location[p]} ${space_name(game.location[p])}: ${rule_text}`,
      );
  }
}
...
function assert_brest_litovsk() {
  const rule_text =
    "Treaty of Brest-Litovsk, Russian units cannot stack with non-Russian AP units";
  ...
  if (game.events.treaty_of_brest_litovsk > 0) {
    for (let s = 1; s < map_space_count; ++s) {
      ...
      if (has_ap_non_russian && has_russian) {
        throw new Error(
          `Rule violation in S${s} ${space_name(s)}: ${rule_text}`,
        );
      }
    }
  }
