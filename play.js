...
function set_mouse_focus(x) {
  if (x === undefined) mouse_focus = 1 - mouse_focus;
  else mouse_focus = x;
...
function card_class_name(card_number) {
  ...
  let faction_card_number =
    card_number > HIGHEST_AP_CARD ? card_number - HIGHEST_AP_CARD : card_number;
  let suffix =
    options.valiant && [27, 48, 62, 130].includes(card_number) ? "_v" : "";
...
function on_focus_card_tip(card_number) {
  document.getElementById("tooltip").className =
    `card ${card_class_name(card_number)}`;
...
function on_click_piece_tip(p) {
  pieces[p].element.scrollIntoView({
    block: "center",
    inline: "center",
    behavior: "smooth",
  });
...
function show_card_list(id, card_lists) {
  show_dialog(id, (body) => {
    ...
    let append_card = (c) => {
      ...
      p.className = "cardtip";
      p.className =
        c <= HIGHEST_AP_CARD ? "cardtip ap-card" : "cardtip cp-card";
...
function get_perm_eliminated() {
  ...
  let war_africa =
    view.removed_cards && view.removed_cards[CP].indexOf(97) >= 0; //war in africa
  ...
  for (let i = 1; i < data.pieces.length; i++) {
    ...
    if (
      view.location[i] === 0 &&
      (p_data.type === "army" || p_data.notreplaceable) &&
      not_placed_pieces.indexOf(i) < 0
    ) {
      result.push(i);
...
function update_perm_elim_box() {
  ...
  let insert_piece_in_stack = function (p) {
    ...
    if (!pe) {
      pe = spaces[PERM_ELIMINATED_BOX].elements[p] = build_unit(
        p,
        document.getElementById("reinforcements"),
      );
    }
    ...
    if (is_corps) unshift_stack(stack, pe);
    else push_stack(stack, pe);
  };

  for_each_piece_in_space(PERM_ELIMINATED_BOX, insert_piece_in_stack);
  get_perm_eliminated().forEach(insert_piece_in_stack);

  let [ap_space_x, ap_space_y] = layout_center_map(
    layout[AP_PER_ELIMINATED_BOX],
    0,
    0,
  );
  let [cp_space_x, cp_space_y] = layout_center_map(
    layout[CP_PER_ELIMINATED_BOX],
    0,
    0,
  );
  ...
  for (let i = 0; i < ap_array.length; i++) {
    ...
    layout_stack(
      ap_space[ap_array[i]],
      ap_space_x + (d % 3) * army_stride,
      ap_space_y + Math.floor(d / 3) * row_stride,
    );
    d++;
  }
  d = 0;
  for (let i = 0; i < cp_array.length; i++) {
    ...
    layout_stack(
      cp_space[cp_array[i]],
      cp_space_x + (d % 3) * army_stride,
      cp_space_y + Math.floor(d / 3) * row_stride,
    );
    d++;
  }
}
...
function update_reinforcements_view() {
  ...
  for (let i = 286; i <= 359; i++) {
    ...
    if (event.status === REMOVED) {
      ...
      let [space_x, space_y] = layout_center(
        layout[space.type + " " + space.event],
      );
...
function flag_supply_warnings() {
  if (!can_flag_supply_warnings()) return;
...
function propose_rollback() {
  if (!can_propose_rollback()) return;
...
function review_rollback() {
  if (!view.rollback_proposal) return;
...
function on_reply(q, params) {
  if (q === "cp_supply") show_cp_supply(params);
  if (q === "ap_supply") show_ap_supply(params);
  if (q === "ap_cards") show_card_list("ap_card_dialog", params);
  if (q === "cp_cards") show_card_list("cp_card_dialog", params);
}
...
function get_control(i) {
  if (view.location[BRITISH_ANA_CORPS] === i) {
    const has_cp_fort =
      spaces[i].fort > 0 &&
      spaces[i].faction === CP &&
      !view.forts.destroyed.includes(i);
...
function show_score_summary() {
  show_dialog("score", (body) => {
    ...
    cp_captured.forEach((s) => {
      append_score(sub_space_name("", s), spaces[s].vp);
    });
    append_header(`AP Captured (-${ap_captured.length})`);
    ap_captured.forEach((s) => {
      append_score(sub_space_name("", s), -spaces[s].vp);
    });
    ...
    view.cp.missed_mo.forEach((turn) => {
      append_score(`Turn ${turn}`, -1);
    });
    append_header(`AP Missed MOs (+${view.ap.missed_mo.length})`);
    view.ap.missed_mo.forEach((turn) => {
      append_score(`Turn ${turn}`, 1);
    });
    ...
    const event_total = score_events.reduce((total, score_event) => {
      return total + score_event[1];
    }, 0);
    append_header(`Score Events (${event_total > 0 ? "+" : ""}${event_total})`);
    score_events.forEach((score_event) => {
      append_score(
        `Turn ${score_event[0]}: ${score_event.length > 2 ? sub_card_name("", score_event[2]) : ""}`,
        score_event[1],
      );
    });
    ...
    if (
      !view.events.reinforcements ||
      !view.events.reinforcements.includes(43)
    ) {
    ...
    if (
      !view.events.reinforcements ||
      !view.events.reinforcements.includes(47)
    ) {
    ...
    append_score(
      "Future Blockade points if game goes to Turn 20",
      -future_blockades,
    );
    ...
    if (view.events.treaty_of_brest_litovsk >= 1) {
      append_string(
        `Treaty of Brest-Litovsk PLAYED: Central Powers win with VP >= 11`,
      );
    } else {
      append_string(
        `Treaty of Brest-Litovsk NOT PLAYED: Central Powers win with VP >= 13`,
      );
    }
  });
}
...
const marker_info = {
  ...
  trench: {
    ap: {
      1: {
        name: "AP Trench Level 1",
        type: "ap_trench_1",
        counter: "marker ap trench_1",
        size: 45,
      },
      2: {
        name: "AP Trench Level 2",
        type: "ap_trench_2",
        counter: "marker ap trench_2",
        size: 45,
      },
    },
    cp: {
      1: {
        name: "CP Trench Level 1",
        type: "cp_trench_1",
        counter: "marker cp trench_1",
        size: 45,
      },
      2: {
        name: "CP Trench Level 2",
        type: "cp_trench_2",
        counter: "marker cp trench_2",
        size: 45,
      },
    },
  },
  ...
  ap_war_status: {
    name: "AP War Status",
    type: "ap_war_status",
    counter: "marker ap war_status",
    size: 45,
  },
  cp_war_status: {
    name: "CP War Status",
    type: "cp_war_status",
    counter: "marker cp war_status",
    size: 45,
  },
  combined_war_status: {
  ...
  ge_rp: {
    name: "German Replacements",
    type: "ge_rp",
    counter: "marker ge_rp",
    size: 45,
  },
  ge_rp_back: {
    name: "German Replacements Rathenau",
    type: "ge_rp_back",
    counter: "marker ge_rp back",
    size: 45,
  },
  ah_rp: {
    name: "Austria-Hungary Replacements",
    type: "ah_rp",
    counter: "marker ah_rp",
    size: 45,
  },
  fr_rp: {
    name: "French Replacements",
    type: "fr_rp",
    counter: "marker fr_rp",
    size: 45,
  },
  br_rp: {
    name: "British Replacements",
    type: "br_rp",
    counter: "marker br_rp",
    size: 45,
  },
  br_rp_back: {
    name: "British Replacements U-Boats",
    type: "br_rp",
    counter: "marker br_rp back",
    size: 45,
  },
  ru_rp: {
    name: "Russian Replacements",
    type: "ru_rp",
    counter: "marker ru_rp",
    size: 45,
  },
  allied_rp: {
    name: "Allied Replacements",
    type: "allied_rp",
    counter: "marker allied_rp",
    size: 45,
  },
  bu_rp: {
    name: "Bulgarian Replacements",
    type: "bu_rp",
    counter: "marker bu_rp",
    size: 45,
  },
  tu_rp: {
    name: "Turkish Replacements",
    type: "tu_rp",
    counter: "marker tu_rp",
    size: 45,
  },
  it_rp: {
    name: "Italian Replacements",
    type: "it_rp",
    counter: "marker it_rp",
    size: 45,
  },
  us_rp: {
    name: "United States Replacements",
    type: "us_rp",
    counter: "marker us_rp",
    size: 45,
  },
  ...
  fort_destroyed_mini: {
    name: "Destroyed Fort Mini",
    counter: "marker mini fort_destroyed",
    size: 18,
  },
  fort_besieged: {
    name: "Besieged Fort",
    counter: "marker fort_besieged",
    size: 45,
  },
  ...
  failed_entrench: {
    name: "Failed Entrench",
    counter: "marker small trench_attempt",
    size: 36,
  },
  mef_beachhead: {
    name: "MEF Beachhead",
    counter: "marker mef_beachhead",
    size: 45,
    cardIndex: 31,
  },
  ...
  fourteen_points: {
    name: "US Points",
    counter: "marker small us_points",
    size: 36,
    cardIndex: 40,
  },
  influenza: {
    name: "Influenza",
    counter: "marker small influenza",
    size: 36,
    cardIndex: 65,
  },
  ...
  blockade: {
    name: "Blockade",
    counter: "marker blockade_vps",
    size: 45,
    cardIndex: 2,
  },
  lusitania: {
    name: "Lusitania",
    counter: "marker lusitania",
    size: 45,
    cardIndex: 26,
  },
  sinai_pipeline: {
    name: "Sinai Pipeline",
    counter: "marker sinai_pipeline",
    size: 45,
    cardIndex: 49,
  },
  sud_army: {
    name: "Sud Army",
    counter: "marker sud_army",
    size: 45,
    cardIndex: 75,
  },
  eleventh_army: {
    name: "11th Army",
    counter: "marker eleventh_army",
    size: 45,
    cardIndex: 94,
  },
  stavka_timidity: {
    name: "Stavka Timidity",
    counter: "marker stavka_timidity",
    size: 45,
    cardIndex: 123,
  },
  ...
  moltke: {
    name: "Moltke",
    counter: "ap-marker marker moltke",
    size: 45,
    cardIndex: 9,
  },
  entrench_ap: {
    name: "Entrench",
    counter: "ap-marker marker entrench",
    size: 45,
    cardIndex: 12,
  },
  rape_of_belgium: {
    name: "Rape of Belgium",
    counter: "ap-marker marker rape_of_belgium",
    size: 45,
    cardIndex: 13,
  },
  great_retreat: {
    name: "Great Retreat",
    counter: "ap-marker marker great_retreat",
    size: 45,
    cardIndex: 27,
  },
  landships: {
    name: "Landships",
    counter: "ap-marker marker landships",
    size: 45,
    cardIndex: 28,
  },
  salonika: {
    name: "Salonika",
    counter: "ap-marker marker salonika",
    size: 45,
    cardIndex: 30,
  },
  ...
  convoy: {
    name: "Convoy",
    counter: "ap-marker marker convoy",
    size: 45,
    cardIndex: 52,
  },
  ...
  over_there: {
    name: "Over There",
    counter: "ap-marker marker over_there",
    size: 45,
    cardIndex: 55,
  },
  grand_fleet: {
    name: "Grand Fleet",
    counter: "ap-marker marker grand_fleet",
    size: 45,
    cardIndex: 33,
  },
  ...
  italy: {
    name: "Italy",
    counter: "ap-marker marker italy",
    size: 45,
    cardIndex: 17,
  },
  romania: {
    name: "Romania",
    counter: "ap-marker marker romania",
    size: 45,
    cardIndex: 16,
  },
  greece: {
    name: "Greece",
    counter: "ap-marker marker greece",
    size: 45,
    cardIndex: 44,
  },
  br1: {
    name: "BR 1",
    counter: "ap-marker marker br1",
    size: 45,
    cardIndex: 14,
  },
  br2: {
    name: "BR 2",
    counter: "ap-marker marker br2",
    size: 45,
    cardIndex: 1,
  },
  mef: {
    name: "MEF",
    counter: "ap-marker marker mef",
    size: 45,
    cardIndex: 31,
  },
  allenby: {
    name: "Allenby",
    counter: "ap-marker marker allenby",
    size: 45,
    cardIndex: 50,
  },
  the_sixtus_affair: {
    name: "The Sixtus Affair",
    counter: "ap-marker marker sixtus",
    size: 45,
    cardIndex: 62,
  },
  ...
  guns_of_august: {
    name: "Guns of August",
    counter: "cp-marker marker guns_of_august",
    size: 45,
    cardIndex: 65 + 1,
  },
  entrench_cp: {
    name: "Entrench",
    counter: "cp-marker marker entrench",
    size: 45,
    cardIndex: 65 + 6,
  },
  race_to_the_sea: {
  ...
  oberost: {
    name: "Oberost",
    counter: "cp-marker marker oberost",
    size: 45,
    cardIndex: 65 + 11,
  },
  reichstag_truce: {
  ...
  falkenhayn: {
    name: "Falkenhayn",
    counter: "cp-marker marker falkenhayn",
    size: 45,
    cardIndex: 65 + 13,
  },
  high_seas_fleet: {
  ...
  zeppelin_raids: {
    name: "Zeppelin Raids",
    counter: "cp-marker marker zeppelin_raids",
    size: 45,
    cardIndex: 65 + 27,
  },
  uboats_unleashed: {
    name: "U-Boats Unleashed",
    counter: "cp-marker marker uboats_unleashed",
    size: 45,
    cardIndex: 65 + 36,
  },
  hoffmann: {
    name: "Hoffmann",
    counter: "cp-marker marker hoffmann",
    size: 45,
    cardIndex: 65 + 37,
  },
  michael: {
    name: "Michael",
    counter: "cp-marker marker michael",
    size: 45,
    cardIndex: 65 + 49,
  },
  blucher: {
    name: "Blucher",
    counter: "cp-marker marker blucher",
    size: 45,
    cardIndex: 65 + 50,
  },
  peace_offensive: {
  ...
  lloyd_george: {
    name: "Lloyd George",
    counter: "cp-marker marker lloyd_george",
    size: 45,
    cardIndex: 65 + 55,
  },
  walter_rathenau: {
    name: "Walter Rathenau",
    counter: "cp-marker marker walter_rathenau",
    size: 45,
    cardIndex: 98,
  },
  place_of_execution: {
  ...
  war_in_africa: {
    name: "War in Africa",
    counter: "cp-marker marker war_in_africa",
    size: 45,
    cardIndex: 97,
  },
  bolshevik_revolution: {
    name: "Bolshevik Revolution",
    counter: "cp-marker marker bolshevik_revolution",
    size: 45,
    cardIndex: 118,
  },
  ...
  french_mutiny: {
    name: "French Mutiny",
    counter: "cp-marker marker french_mutiny",
    size: 45,
    cardIndex: 112,
  },
  ...
  bulgaria: {
    name: "Bulgaria",
    counter: "cp-marker marker bulgaria",
    size: 45,
    cardIndex: 99,
  },
};
...
function for_each_piece_in_space(s, fun) {
  for (let p = 1; p < pieces.length; ++p)
    if (abs(view.location[p]) === s) fun(p);
}
...
function on_click_space(evt) {
  if (evt.button === 0) {
    ...
    if (
      view.actions &&
      view.actions.space &&
      view.actions.space.includes(space)
    ) {
      if (send_action("space", space)) event.stopPropagation();
    } else if (
      view.actions &&
      (view.actions.activate_move ||
        view.actions.activate_attack ||
        view.actions.activate_attack_mutiny ||
      ...
      if (options.length > 0) {
        let hide =
          view.actions.activate_attack_mutiny &&
          view.actions.activate_attack_mutiny.includes(space)
            ? "activate_attack"
...
function on_focus_space(evt) {
  ...
  if (DEBUG_SPACES) {
    ...
    else text += ` (${space.nation.toUpperCase()})`;
...
function stack_piece_count(stack) {
  let n = 0;
  for (let i = 0; i < stack.length; ++i) if (stack[i][0] > 0) ++n;
  return n;
}
...
function on_focus_piece(evt) {
  ...
  if (mouse_focus) focus_stack(evt.target.my_stack);
}
...
function format_card_info(c) {
  ...
  if (card.remove) text += "*";
  if (card.ws) text += ` (${card.ws})`;
  if (card.cc) text += ` <span style="color:red">CC</span>`;
  return text;
}
...
let card_action_menu = Array.from(
  document.getElementById("card_popup").querySelectorAll("li[data-action]"),
...
function show_popup_menu(evt, menu_id, target_id, title, hide = "") {
  ...
  if (show) {
    ...
    let x = Math.max(
      5,
      Math.min(evt.clientX - w / 2, window.innerWidth - w - 5),
    );
    let y = Math.max(
      5,
      Math.min(evt.clientY - 12, window.innerHeight - h - 40),
    );
...
function is_card_enabled(card) {
  if (view.actions) {
    if (
      card_action_menu.some(
        (a) => view.actions[a] && view.actions[a].includes(card),
      )
    )
      return true;
    if (view.actions.card && view.actions.card.includes(card)) return true;
  }
  return false;
}
...
function is_action(action, card) {
  return !!(
    view.actions &&
    view.actions[action] &&
    view.actions[action].includes(card)
  );
}
...
ui.russian_capitulation = build_unique_marker(
  "marker small russian_capitulation",
  36,
);
...
function build_marker(list, find, new_marker, info, no_listeners) {
  ...
  if (marker) return marker.element;
...
function build_fort_destroyed_marker(space_id) {
  return build_marker(
    markers.forts.destroyed,
    (e) => e.space_id === space_id,
    { space_id: space_id },
    marker_info.fort_destroyed,
  );
}
...
function build_fort_destroyed_mini_marker(space_id) {
  return build_marker(
    markers.forts.destroyed_mini,
    (e) => e.space_id === space_id,
    { space_id: space_id },
    marker_info.fort_destroyed_mini,
    true,
  );
}
...
function build_fort_besieged_marker(space_id) {
  return build_marker(
    markers.forts.besieged,
    (e) => e.space_id === space_id,
    { space_id: space_id },
    marker_info.fort_besieged,
  );
}
...
function build_turn_track_marker(type) {
  ...
  if (marker_info[type].cardIndex && marker_info[type].cardIndex > 0) {
    marker.setAttribute(
      "onmouseenter",
      `on_focus_card_tip(${marker_info[type].cardIndex})`,
    );
...
function build_missed_mo_marker(faction, turn) {
  return build_marker(
    markers.missed_mo[faction],
    (e) => e.turn === turn,
    { turn: turn },
    marker_info[faction + "_missed_mo"],
  );
}
...
function build_failed_entrench_marker(piece_id) {
  return build_marker(
    markers.failed_entrench,
    (e) => e.piece_id === piece_id,
    { piece_id: piece_id, space_id: view.location[piece_id] },
    marker_info.failed_entrench,
  );
}
...
function build_action_marker(faction, round) {
  let elt = build_marker(
    markers.actions,
    (e) => e.faction === faction && e.round === round,
    {
      faction: faction,
      round: round,
    },
...
function layout_center(rect, dx = 0, dy = 0) {
  if (!rect || !Array.isArray(rect) || rect.length < 4) return [0, 0];
  let [x, y, w, h] = rect;
  return [Math.floor(x + w / 2) + dx, Math.ceil(y + h / 2) + dy];
}

/* Map 9995×6951 with contain → 2372×1650.
 * Layout 0..2550 → display. Add inset so left/right edges align (center stays). */
const MAP_DISPLAY_WIDTH = 2372;
const MAP_X_INSET = -90;
const MAP_X_OFFSET = MAP_X_INSET;
const MAP_X_SCALE = (MAP_DISPLAY_WIDTH - 2 * MAP_X_INSET) / 2550;
function map_x(x) {
  return MAP_X_OFFSET + Math.round(x * MAP_X_SCALE);
}
function init_map_scale_css() {
  const overlay = document.getElementById("map-overlay");
  if (overlay) {
    overlay.style.setProperty("--map-x-offset", String(MAP_X_OFFSET));
    overlay.style.setProperty("--map-x-scale", String(MAP_X_SCALE));
  }
}
init_map_scale_css();

function layout_center_map(rect, dx, dy) {
  let [x, y] = layout_center(rect, dx, dy);
  return [map_x(x), y];
}
const layout_ap_mo = {
  fr: layout_center_map(layout["AP MO FR"], 0, -35),
  br: layout_center_map(layout["AP MO BR"], 0, -35),
  it: layout_center_map(layout["AP MO IT"], 0, -35),
  ru: layout_center_map(layout["AP MO RU GE"], 0, -35),
  ru_ge: layout_center_map(layout["AP MO RU GE"], 0, -35),
  ru_ah: layout_center_map(layout["AP MO RU AH"], 0, -35),
  none: layout_center_map(layout["AP MO None"], 0, -35),
};

const layout_cp_mo = {
  ah: layout_center_map(layout["CP MO AH"], 0, 35),
  ah_it: layout_center_map(layout["CP MO IT"], 0, 35),
  tu: layout_center_map(layout["CP MO TU"], 0, 35),
  ge: layout_center_map(layout["CP MO GE"], 0, 35),
  none: layout_center_map(layout["CP MO None"], 0, 35),
};

function build_space(id, space_layout, is_map_space = true) {
  let space = spaces[id];
  let [x, y, w, h] = space_layout
    ? space_layout
    : grow_layout(layout[space.name], 8);
  if (is_map_space) x = map_x(x);
  ...
  elt.space = id;
  if (space.fort && space.vp) elt.className = "space fort vp";
  else if (space.fort) elt.className = "space fort";
  else if (space.vp) elt.className = "space vp";
  else if (
    space.name === "MEF1" ||
    space.name === "MEF2" ||
    space.name === "MEF3" ||
...
function build_ws_event_space(id, units) {
  ...
  let elt = build_space(id, layout[space.type + " " + space.event], false);
  ...
  if (units && space.type === "reinf") {
    let insert_piece_in_stack = function (p) {
      ...
      if (is_corps) unshift_stack(space.stack, pe);
      else push_stack(space.stack, pe);
    };
...
function build_unit(id, place) {
  ...
  if (unit.type === "army") elt.my_size = 45;
  else elt.my_size = 36;
...
for (let s = 1; s < spaces.length; ++s) {
  if (s === AP_RESERVE_BOX || s === CP_RESERVE_BOX) build_reserve_box(s);
  else if (s === AP_ELIMINATED_BOX || s === CP_ELIMINATED_BOX)
    build_eliminated_box(s);
  else if (!spaces[s].hidden) build_space(s);
}
build_perm_elim_box();
...
function is_action_piece(p) {
  if (view.actions && view.actions.piece && view.actions.piece.includes(p))
    return true;
  if (view.who === p) return true;
  return false;
}

function is_different_piece(a, b) {
  if (a > 0 && b > 0) {
    if (pieces[a].type !== pieces[b].type) return true;
    if (view.reduced.includes(a) !== view.reduced.includes(b)) return true;
    return false;
  }
  return true;
}
...
function layout_stack(stack, start_x, start_y) {
  ...
  if (stack === focus && is_small_stack(stack)) focus = null;
...
function update_space(s) {
  ...
  for_each_piece_in_space(s, (p) => {
    ...
    if (is_reduced) pe.classList.add("reduced");
    else pe.classList.remove("reduced");
  ...
  let ordered_pieces = full_armies
    .concat(reduced_armies)
    .concat(full_corps)
    .concat(reduced_corps);
  ordered_pieces.forEach((p) => {
    ...
    if (
      view.oos_pieces &&
      view.oos_pieces.length > 0 &&
      view.oos_pieces.includes(p)
    ) {
  ...
  if (view.mef_beachhead === s) {
    push_stack(
      stack,
      build_marker(
        markers.mef_beachhead,
        (e) => e.space_id === s,
        { space_id: s },
        marker_info.mef_beachhead,
      ),
    );
  } else {
  ...
  if (
    (space.faction !== get_control(s) && count_pieces === 0) ||
    is_broken_siege
  )
  ...
  if (view.activated.move.includes(s)) {
    let markers = view.activation_cost
      ? map_get(view.activation_cost, s, 1)
      : 1;
...
function update_reserve_boxes() {
  ...
  let [ap_space_x, ap_space_y] = layout_center_map(layout[ap_space.name], 0, 0);
  let [cp_space_x, cp_space_y] = layout_center_map(layout[cp_space.name], 0, 0);
  ...
  let insert_piece_in_stack = function (p) {
    ...
    pe.classList.remove("offmap");
    pe.classList.remove("inside");
    if (view.reduced.includes(p)) pe.classList.add("reduced");
    else pe.classList.remove("reduced");

    const nation = pieces[p].nation;
    const space = pieces[p].faction === CP ? cp_space : ap_space;
    const which_stack = get_reserve_box_stack(nation, p);
    let stack = view.reduced.includes(p)
      ? space.stacks[which_stack].reduced
      : space.stacks[which_stack].full;
    if (is_corps) unshift_stack(stack, pe);
    else push_stack(stack, pe);
  };
  ...
  for (let i = 0; i < ap_reserve_box_order.length; ++i) {
    ...
    if (ap_space.stacks[nation].reduced.length > 0) {
      layout_stack(
        ap_space.stacks[nation].reduced,
        ap_x + i * stride,
        ap_y + stride,
      );
    }
  ...
  for (let i = 0; i < cp_reserve_box_order.length; ++i) {
    ...
    if (cp_space.stacks[nation].reduced.length > 0) {
      layout_stack(
        cp_space.stacks[nation].reduced,
        cp_x + i * stride,
        cp_y + stride,
      );
    }
...
function update_eliminated_boxes() {
  ...
  let insert_piece_in_stack = function (p) {
    ...
    const space = pieces[p].faction === CP ? cp_space : ap_space;
    let stack = is_corps
      ? space.stacks[get_eliminated_box_group(p)].corps
      : space.stacks[get_eliminated_box_group(p)].armies;
    unshift_stack(stack, pe);
  };
  ...
  for_each_piece_in_space(0, (p) => {
    if (is_action_piece(p)) insert_piece_in_stack(p);
  });
  ...
  for (let i = 0; i < ap_eliminated_box_order.length; ++i) {
    ...
    if (ap_space.stacks[group].armies.length > 0) {
      layout_stack(
        ap_space.stacks[group].armies,
        ap_army_x + i * army_stride,
        ap_y,
      );
    }
    if (ap_space.stacks[group].corps.length > 0) {
      layout_stack(
        ap_space.stacks[group].corps,
        ap_corp_x + i * corp_stride,
        ap_y + row_stride,
      );
    }
  }
  ...
  for (let i = 0; i < cp_eliminated_box_order.length; ++i) {
    ...
    if (cp_space.stacks[group].armies.length > 0) {
      layout_stack(
        cp_space.stacks[group].armies,
        cp_army_x + i * army_stride,
        cp_y,
      );
    }
    if (cp_space.stacks[group].corps.length > 0) {
      layout_stack(
        cp_space.stacks[group].corps,
        cp_corp_x + i * corp_stride,
        cp_y + row_stride,
      );
    }
  }
...
function should_highlight_space(s) {
  ...
  if (view.actions.space && view.actions.space.includes(s)) return true;
  ...
  if (
    view.actions.activate_attack_mutiny &&
    view.actions.activate_attack_mutiny.includes(s)
  )
    return true;
...
function update_card(id) {
  ...
  elt.classList.toggle(
    "highlight",
    is_action("card", id) || is_action("play_event", id),
  );
}
...
function update_piece(id) {
  ...
  piece.element.classList.toggle(
    "highlight",
    !!(view.actions && view.actions.piece && view.actions.piece.includes(id)),
  );
  piece.element.classList.toggle(
    "activated",
    !!(
  ...
  piece.element.classList.toggle(
    ...
    !!(
      (view.attack &&
        view.attack.advancing_pieces &&
        view.attack.advancing_pieces.includes(id)) ||
      (view.attack &&
        view.attack.retreating_pieces &&
        view.attack.retreating_pieces.includes(id)) ||
      (view.move && view.move.pieces.includes(id)) ||
      view.who === id
    ) && !is_action("piece", id),
  );
  piece.element.classList.toggle(
    "entrenching",
    !!(view.entrenching && view.entrenching.includes(id)),
  );
}
...
function update_turn_track() {
  ...
  update_turn_track_marker(
    "italy",
    view.war[ITALY],
    !view.war[ITALY] || view.war[ITALY] === 1,
  );
  update_turn_track_marker(
    "romania",
    view.war[ROMANIA],
    !view.war[ROMANIA] || view.war[ROMANIA] === 1,
  );
  update_turn_track_marker(
    "bulgaria",
    view.war[BULGARIA],
    !view.war[BULGARIA] || view.war[BULGARIA] === 1,
  );
  update_turn_track_marker(
    "greece",
    view.war[GREECE],
    !view.war[GREECE] || view.war[GREECE] === 1,
  );
  ...
  turn_track_stacks.forEach((stack, ix) => {
    if (stack.length > 0) {
      let [x, y] = layout_center_map(layout["Turn " + (ix + 1)], 0, 0);
...
function update_general_records_track() {
  ...
  general_records_stacks.forEach((stack, ix) => {
    if (stack.length > 0) {
      let [x, y] = layout_center_map(layout["GR " + ix], 0, 0);
      layout_stack(stack, x, y);
    }
  });
}
...
function update_ne_limits() {
  if (view.ne_limits.br_sr) ui.ne_limits.br_sr.classList.add("used");
  else ui.ne_limits.br_sr.classList.remove("used");

  if (view.ne_limits.cp_sr) ui.ne_limits.cp_sr.classList.add("used");
  else ui.ne_limits.cp_sr.classList.remove("used");

  if (view.ne_limits.ru_sr) ui.ne_limits.ru_sr.classList.add("used");
  else ui.ne_limits.ru_sr.classList.remove("used");

  if (view.ne_limits.ru_non_sr) ui.ne_limits.ru_move.classList.add("used");
  else ui.ne_limits.ru_move.classList.remove("used");
}
...
let action_stacks = {
  ap: {
    entry: {
      xy: layout_center_map(layout["AP AR Neutral Entry"], 0, 0),
      stack: [],
    },
    reinf_fr: {
      xy: layout_center_map(layout["AP AR FR Reinf"], 0, 0),
      stack: [],
    },
    reinf_br: {
      xy: layout_center_map(layout["AP AR BR Reinf"], 0, 0),
      stack: [],
    },
    reinf_ru: {
      xy: layout_center_map(layout["AP AR RU Reinf"], 0, 0),
      stack: [],
    },
    reinf_it: {
      xy: layout_center_map(layout["AP AR IT Reinf"], 0, 0),
      stack: [],
    },
    reinf_us: {
      xy: layout_center_map(layout["AP AR US Reinf"], 0, 0),
      stack: [],
    },
    sr: { xy: layout_center_map(layout["AP AR SR Card"], 0, 0), stack: [] },
    rp: { xy: layout_center_map(layout["AP AR RP Card"], 0, 0), stack: [] },
    op: { xy: layout_center_map(layout["AP AR OPS Card"], 0, 0), stack: [] },
    evt: {
      xy: layout_center_map(layout["AP AR Other Event"], 0, 0),
      stack: [],
    },
    oneop: { xy: layout_center_map(layout["AP AR 1 Ops"], 0, 0), stack: [] },
    peace: {
      xy: layout_center_map(layout["AP AR Peace Terms"], 0, 0),
      stack: [],
    },
  },
  cp: {
    entry: {
      xy: layout_center_map(layout["CP AR Neutral Entry"], 0, 0),
      stack: [],
    },
    reinf_ge: {
      xy: layout_center_map(layout["CP AR GE Reinf"], 0, 0),
      stack: [],
    },
    reinf_ah: {
      xy: layout_center_map(layout["CP AR AH Reinf"], 0, 0),
      stack: [],
    },
    reinf_tu: {
      xy: layout_center_map(layout["CP AR TU Reinf"], 0, 0),
      stack: [],
    },
    sr: { xy: layout_center_map(layout["CP AR SR Card"], 0, 0), stack: [] },
    rp: { xy: layout_center_map(layout["CP AR RP Card"], 0, 0), stack: [] },
    op: { xy: layout_center_map(layout["CP AR OPS Card"], 0, 0), stack: [] },
    evt: {
      xy: layout_center_map(layout["CP AR Other Event"], 0, 0),
      stack: [],
    },
    oneop: { xy: layout_center_map(layout["CP AR 1 Ops"], 0, 0), stack: [] },
    peace: {
      xy: layout_center_map(layout["CP AR Peace Terms"], 0, 0),
      stack: [],
    },
  },
};
...
function update_violations() {
  if (view.violations.length > 0) {
    ...
    for (let v of view.violations) {
      let p = document.createElement("div");
      if (v.space > 0) p.innerHTML = escape_text(`s${v.space}: ${v.rule}`);
      else if (v.piece > 0) p.innerHTML = escape_text(`P${v.piece}: ${v.rule}`);
      else p.innerHTML = escape_text(v.rule);
      ui.violations.appendChild(p);
    }
  } else {
    ui.violations.replaceChildren();
...
function on_log(text, ix) {
  ...
  if (text.startsWith("!")) {
    ...
    text = text.substring(4);
    if (text === "AP") p.className = "h2 ap";
    else if (text === "CP") p.className = "h2 cp";
    else p.className = "h2";
  ...
  if (log_box_ap) p.classList.add("group", "ap");
  if (log_box_cp) p.classList.add("group", "cp");
...
function escape_text(text) {
  ...
  text = text.replace(
    /\+\d VP/g,
    (match) => `<span class="cpvp">${match}</span>`,
  );
  text = text.replace(
    /[-−]\d VP/g,
    (match) => `<span class="apvp">${match}</span>`,
  );
  return text;
}
...
function update_map() {
  ...
  for_each_piece_in_space(PERM_ELIMINATED_BOX, (p) =>
    pieces[p].element.classList.add("offmap"),
  );

  update_card_zones();

  for (let i = 1; i < cards.length; ++i) update_card(i);
  for (let i = 1; i < AP_RESERVE_BOX; ++i) update_space(i, false);
  update_reserve_boxes();
  update_eliminated_boxes();
  for (let i = 0; i < pieces.length; ++i) update_piece(i);

  if (focus && focus.length <= 1) focus = null;
  ...
  update_unique_marker(
    ui.us_entry,
    layout_center_map(layout["US Track " + (view.us_entry + 1)], 0, 0),
  );
  update_unique_marker(
    ui.russian_capitulation,
    layout_center_map(
      layout["RC Track " + (view.russian_capitulation + 1)],
      0,
      0,
    ),
  );
  ...
  update_turn_track();
  // update_action_round_tracks();  // commented out: 不加载 action_stacks 的 layout
  ...
  document.getElementById("ap_deck_size").textContent =
    `Allied Powers deck: ${view.ap.deck} cards`;
  document.getElementById("cp_deck_size").textContent =
    `Central Powers deck: ${view.cp.deck} cards`;
  ...
  let flag_supply_warning_menu = document.getElementById(
    "flag_supply_warning_menu",
  );
  ...
  if (
    view.rollback_proposal &&
    view.actions &&
    "accept" in view.actions &&
    "reject" in view.actions
  ) {
...
function update_card_zones() {
  ...
  if (view.hand)
    for (let i of view.hand) ui.cards.appendChild(cards[i].element);
  ...
  if (view.attack) {
    ...
    for (let i of view.combat_cards)
      ...
      else ui.unused_combat_cards.appendChild(cards[i].element);
  ...
  if (
    view.events.lloyd_george === view.turn &&
    !view.events.lloyd_george_canceled
  ) {
    show_active_cards = true;
...
function on_snap_penultimate() {
  ...
  if (view.attack !== null) on_snap_prev();
  else request_snap(snap_count - 1, on_snap_prev);
}
